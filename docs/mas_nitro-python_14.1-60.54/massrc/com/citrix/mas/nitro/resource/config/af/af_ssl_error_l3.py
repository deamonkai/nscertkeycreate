...
'''
Configuration for AF SSL ERROR Report table for Level 3 resource
'''

class af_ssl_error_l3(base_resource):
	_rpt_sample_time= ""
	_vservername= ""
	_ip_address= ""
	_mismatchciphers= ""
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				af_ssl_error_l3_obj=af_ssl_error_l3()
				response = af_ssl_error_l3_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of af_ssl_error_l3 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return af_ssl_error_l3_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the af_ssl_error_l3 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = af_ssl_error_l3_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of af_ssl_error_l3 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = af_ssl_error_l3_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_ssl_error_l3_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_ssl_error_l3_responses, response, "af_ssl_error_l3_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.af_ssl_error_l3_response_array
				i=0
				error = [af_ssl_error_l3() for _ in range(len(response))]
				for obj in response :
			...
			response = result.af_ssl_error_l3_response_array
			i=0
			af_ssl_error_l3_objs = [af_ssl_error_l3() for _ in range(len(response))]
			for obj in response :
				for props in obj._af_ssl_error_l3:
					result = service.payload_formatter.string_to_bulk_resource(af_ssl_error_l3_response,self.__class__.__name__,props)
					af_ssl_error_l3_objs[i] = result.af_ssl_error_l3
					i=i+1
			return af_ssl_error_l3_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(af_ssl_error_l3,self).validate()
		except Exception as e :
			raise e
...
'''
class af_ssl_error_l3_response(base_response):
	def __init__(self,length=1) :
		self.af_ssl_error_l3= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.af_ssl_error_l3= [ af_ssl_error_l3() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class af_ssl_error_l3_responses(base_response):
	def __init__(self,length=1) :
		self.af_ssl_error_l3_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.af_ssl_error_l3_response_array = [ af_ssl_error_l3() for _ in range(length)]
