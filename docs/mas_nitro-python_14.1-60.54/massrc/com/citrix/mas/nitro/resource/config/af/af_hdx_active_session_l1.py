...
class af_hdx_active_session_l1(base_resource):
	_rpt_sample_time= ""
	_client_jitter= ""
	_server_delay= ""
	_client_ip= ""
	...
	def get_object_type(self) :
		try:
			return "af_hdx_active_session_l1"
	...
	def file_component_value(self) :
		try :
			return "af_hdx_active_session_l1s"
		except Exception as e :
			raise e
	...
	def server_total_rtt(self,server_total_rtt):
		try :
			if not isinstance(server_total_rtt,int):
				raise TypeError("server_total_rtt must be set to int value")
	...
	def client_srtt(self,client_srtt):
		try :
			if not isinstance(client_srtt,int):
				raise TypeError("client_srtt must be set to int value")
	...
	def ica_rtt(self,ica_rtt):
		try :
			if not isinstance(ica_rtt,int):
				raise TypeError("ica_rtt must be set to int value")
	...
	def client_total_rtt(self,client_total_rtt):
		try :
			if not isinstance(client_total_rtt,int):
				raise TypeError("client_total_rtt must be set to int value")
	...
	def server_srtt(self,server_srtt):
		try :
			if not isinstance(server_srtt,int):
				raise TypeError("server_srtt must be set to int value")
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				af_hdx_active_session_l1_obj=af_hdx_active_session_l1()
				response = af_hdx_active_session_l1_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of af_hdx_active_session_l1 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return af_hdx_active_session_l1_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the af_hdx_active_session_l1 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = af_hdx_active_session_l1_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of af_hdx_active_session_l1 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = af_hdx_active_session_l1_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_hdx_active_session_l1_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.af_hdx_active_session_l1
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_hdx_active_session_l1_responses, response, "af_hdx_active_session_l1_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.af_hdx_active_session_l1_response_array
				i=0
				error = [af_hdx_active_session_l1() for _ in range(len(response))]
				for obj in response :
			...
			response = result.af_hdx_active_session_l1_response_array
			i=0
			af_hdx_active_session_l1_objs = [af_hdx_active_session_l1() for _ in range(len(response))]
			for obj in response :
				for props in obj._af_hdx_active_session_l1:
					result = service.payload_formatter.string_to_bulk_resource(af_hdx_active_session_l1_response,self.__class__.__name__,props)
					af_hdx_active_session_l1_objs[i] = result.af_hdx_active_session_l1
					i=i+1
			return af_hdx_active_session_l1_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(af_hdx_active_session_l1,self).validate()
		except Exception as e :
			raise e
...
'''
class af_hdx_active_session_l1_response(base_response):
	def __init__(self,length=1) :
		self.af_hdx_active_session_l1= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.af_hdx_active_session_l1= [ af_hdx_active_session_l1() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class af_hdx_active_session_l1_responses(base_response):
	def __init__(self,length=1) :
		self.af_hdx_active_session_l1_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.af_hdx_active_session_l1_response_array = [ af_hdx_active_session_l1() for _ in range(length)]
