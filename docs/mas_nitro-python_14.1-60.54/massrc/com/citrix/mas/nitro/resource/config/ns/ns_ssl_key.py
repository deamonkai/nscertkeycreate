...
'''
Configuration for NetScaler SSL key File resource
'''
...
class ns_ssl_key(base_resource):
	...
	'''
	@ec_curve.setter
	def ec_curve(self,ec_curve):
		try :
			if not isinstance(ec_curve,str):
				raise TypeError("ec_curve must be set to str value")
			self._ec_curve = ec_curve
		except Exception as e :
			raise e
	...
	def create(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				res = resource.perform_operation(service,"create")
				return res
			else : 
