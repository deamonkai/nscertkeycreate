...
'''
Configuration for SSL certificate configuration resource
'''
...
class ssl_cert_threshold(threshold):
	...
	'''
	get Set the value to true if default certs in device need not be managed
	'''
	@property
	def hide_default_certs(self) :
		try:
			return self._hide_default_certs
		except Exception as e :
			raise e
	'''
	set Set the value to true if default certs in device need not be managed
	'''
	@hide_default_certs.setter
	def hide_default_certs(self,hide_default_certs):
		try :
			if not isinstance(hide_default_certs,bool):
				raise TypeError("hide_default_certs must be set to bool value")
			self._hide_default_certs = hide_default_certs
		except Exception as e :
			raise e
