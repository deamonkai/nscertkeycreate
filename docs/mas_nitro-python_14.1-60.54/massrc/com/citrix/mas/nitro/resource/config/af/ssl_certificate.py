...
class ssl_certificate(af_generic_api):
	_server_ip_address= ""
	_name= ""
	_application_name= ""
	_total_requests= ""
	...
	def get_object_id(self) :
		try:
			return super(ssl_certificate,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ssl_certificate_obj=ssl_certificate()
				response = ssl_certificate_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ssl_certificate resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ssl_certificate_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ssl_certificate resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ssl_certificate_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ssl_certificate resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ssl_certificate_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ssl_certificate_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ssl_certificate_responses, response, "ssl_certificate_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ssl_certificate_response_array
				i=0
				error = [ssl_certificate() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ssl_certificate_response_array
			i=0
			ssl_certificate_objs = [ssl_certificate() for _ in range(len(response))]
			for obj in response :
				for props in obj._ssl_certificate:
					result = service.payload_formatter.string_to_bulk_resource(ssl_certificate_response,self.__class__.__name__,props)
					ssl_certificate_objs[i] = result.ssl_certificate
					i=i+1
			return ssl_certificate_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ssl_certificate,self).validate()
		except Exception as e :
			raise e
...
'''
class ssl_certificate_response(base_response):
	def __init__(self,length=1) :
		self.ssl_certificate= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ssl_certificate= [ ssl_certificate() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ssl_certificate_responses(base_response):
	def __init__(self,length=1) :
		self.ssl_certificate_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ssl_certificate_response_array = [ ssl_certificate() for _ in range(length)]
