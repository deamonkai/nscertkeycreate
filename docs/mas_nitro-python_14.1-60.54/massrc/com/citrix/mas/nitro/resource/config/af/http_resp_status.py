...
class http_resp_status(af_generic_api):
	_cache_bypass= ""
	_ic_hits= ""
	_client_ip_address= ""
	_ctnsappname_b= ""
	...
	def get_object_id(self) :
		try:
			return super(http_resp_status,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Get all Af Report for HTTP Response Status..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				http_resp_status_obj=http_resp_status()
				response = http_resp_status_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of http_resp_status resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return http_resp_status_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the http_resp_status resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = http_resp_status_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of http_resp_status resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = http_resp_status_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(http_resp_status_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.http_resp_status
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(http_resp_status_responses, response, "http_resp_status_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.http_resp_status_response_array
				i=0
				error = [http_resp_status() for _ in range(len(response))]
				for obj in response :
			...
			response = result.http_resp_status_response_array
			i=0
			http_resp_status_objs = [http_resp_status() for _ in range(len(response))]
			for obj in response :
				for props in obj._http_resp_status:
					result = service.payload_formatter.string_to_bulk_resource(http_resp_status_response,self.__class__.__name__,props)
					http_resp_status_objs[i] = result.http_resp_status
					i=i+1
			return http_resp_status_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(http_resp_status,self).validate()
		except Exception as e :
			raise e
...
'''
class http_resp_status_response(base_response):
	def __init__(self,length=1) :
		self.http_resp_status= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.http_resp_status= [ http_resp_status() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class http_resp_status_responses(base_response):
	def __init__(self,length=1) :
		self.http_resp_status_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.http_resp_status_response_array = [ http_resp_status() for _ in range(length)]
