...
'''
Configuration for Applications Hit Counters Report resource
'''

class policy_application(af_generic_api):
	_policy_name= ""
	_pcp_hits= ""
	_ns_ip_address= ""
	_rpt_sample_time= ""
	...
	def get_object_id(self) :
		try:
			return super(policy_application,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				policy_application_obj=policy_application()
				response = policy_application_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of policy_application resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return policy_application_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the policy_application resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = policy_application_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of policy_application resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = policy_application_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(policy_application_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.policy_application
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(policy_application_responses, response, "policy_application_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.policy_application_response_array
				i=0
				error = [policy_application() for _ in range(len(response))]
				for obj in response :
			...
			response = result.policy_application_response_array
			i=0
			policy_application_objs = [policy_application() for _ in range(len(response))]
			for obj in response :
				for props in obj._policy_application:
					result = service.payload_formatter.string_to_bulk_resource(policy_application_response,self.__class__.__name__,props)
					policy_application_objs[i] = result.policy_application
					i=i+1
			return policy_application_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(policy_application,self).validate()
		except Exception as e :
			raise e
...
'''
class policy_application_response(base_response):
	def __init__(self,length=1) :
		self.policy_application= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.policy_application= [ policy_application() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class policy_application_responses(base_response):
	def __init__(self,length=1) :
		self.policy_application_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.policy_application_response_array = [ policy_application() for _ in range(length)]
