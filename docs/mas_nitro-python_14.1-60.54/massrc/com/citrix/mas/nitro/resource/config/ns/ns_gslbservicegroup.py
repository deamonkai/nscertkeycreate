...
'''
Configuration for NetScaler GSLB Service Group Information resource
'''

class ns_gslbservicegroup(base_resource):
	_svc_grp_mem_port= ""
	_partition_name= ""
	_appflowlog= ""
	_display_name= ""
	...
	'''
	get AppFlow is enabled at GSLB servicegroup of GSLBVServer
	'''
	@property
	def appflowlog(self) :
		try:
			...
			raise e
	'''
	set AppFlow is enabled at GSLB servicegroup of GSLBVServer
	'''
	@appflowlog.setter
	...
	'''
	@svr_name.setter
	def svr_name(self,svr_name):
		try :
			if not isinstance(svr_name,str):
				raise TypeError("svr_name must be set to str value")
	...
	'''
	@svr_id.setter
	def svr_id(self,svr_id):
		try :
			if not isinstance(svr_id,str):
				raise TypeError("svr_id must be set to str value")
			self._svr_id = svr_id
		except Exception as e :
			raise e
	...
	'''
	Use this operation to poll the GSLB service group.
	'''
	@classmethod
	...
	'''
	Use this operation to enable the GSLB service group.
	'''
	@classmethod
	...
	'''
	Use this operation to get GSLB Service Group Information.
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ns_gslbservicegroup_obj=ns_gslbservicegroup()
				response = ns_gslbservicegroup_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this operation to disable the GSLB service group.
	'''
	@classmethod
	...
	'''
	Use this API to fetch filtered set of ns_gslbservicegroup resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ns_gslbservicegroup_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ns_gslbservicegroup resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ns_gslbservicegroup_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ns_gslbservicegroup resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ns_gslbservicegroup_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_gslbservicegroup_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.ns_gslbservicegroup
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_gslbservicegroup_responses, response, "ns_gslbservicegroup_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ns_gslbservicegroup_response_array
				i=0
				error = [ns_gslbservicegroup() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ns_gslbservicegroup_response_array
			i=0
			ns_gslbservicegroup_objs = [ns_gslbservicegroup() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_ns_gslbservicegroup'):
					for props in obj._ns_gslbservicegroup:
						result = service.payload_formatter.string_to_bulk_resource(ns_gslbservicegroup_response,self.__class__.__name__,props)
						ns_gslbservicegroup_objs[i] = result.ns_gslbservicegroup
						i=i+1
			return ns_gslbservicegroup_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ns_gslbservicegroup,self).validate()
		except Exception as e :
			raise e
...
'''
class ns_gslbservicegroup_response(base_response):
	def __init__(self,length=1) :
		self.ns_gslbservicegroup= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ns_gslbservicegroup= [ ns_gslbservicegroup() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ns_gslbservicegroup_responses(base_response):
	def __init__(self,length=1) :
		self.ns_gslbservicegroup_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ns_gslbservicegroup_response_array = [ ns_gslbservicegroup() for _ in range(length)]
