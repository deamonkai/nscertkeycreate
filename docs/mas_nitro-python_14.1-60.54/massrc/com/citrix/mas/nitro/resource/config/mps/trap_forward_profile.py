...
class trap_forward_profile(base_resource):
	...
	'''
	get SNMP community name using which we want to forward the trap
	'''
	@property
	def trap_community(self) :
		try:
			...
			raise e
	'''
	set SNMP community name using which we want to forward the trap
	'''
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				return cls.perform_operation_bulk_request(service,resource,trap_forward_profile_obj)
		except Exception as e :
			raise e
