...
class graph(base_resource):
	...
	def get_namespace_url(self) :
		try:
			namespace_url = ""
			object_types_list = ["stylebooks","configpacks","graph"]
	...
	'''
	get The attributes of the object created
	'''
	@property
	def properties(self) :
		try:
			...
			raise e
	'''
	set The attributes of the object created
	'''
	@properties.setter
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				graph_obj=graph()
				response = graph_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of graph resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return graph_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the graph resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			cls._request_params = []
			cls._response_params = []
			cls._response_params = ["type","properties"]
			graph_obj = graph()
			option_ = options()
			option_._count=True
			response = graph_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of graph resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = graph_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(graph_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(graph_responses, response, "graph_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.graph_response_array
				i=0
				error = [graph() for _ in range(len(response))]
				for obj in response :
			...
			response = result.graph_response_array
			i=0
			graph_objs = [graph() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_graph'):
					for props in obj._graph:
						result = service.payload_formatter.string_to_bulk_resource(graph_response,self.__class__.__name__,props)
						graph_objs[i] = result.graph
						i=i+1
			return graph_objs
		except Exception as e :
			raise e
...
'''
class graph_response(base_response):
	def __init__(self,length=1) :
		self.graph= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.graph= [ graph() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class graph_responses(base_response):
	def __init__(self,length=1) :
		self.graph_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.graph_response_array = [ graph() for _ in range(length)]
