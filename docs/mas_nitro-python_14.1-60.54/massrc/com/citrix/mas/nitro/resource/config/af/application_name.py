...
'''
Configuration for AF Application Report table resource
'''

class application_name(af_generic_api):
	_name= ""
	...
	_rpt_sample_time= ""
	_ic_miss= ""
	_ssl_cipher_failure_backend= ""
	_domain_name= ""
	_unsupportedCiphers= ""
	_ssl_cipher_failure_frontend= ""
	_percent_bw_saved= ""
	_ip_address= ""
	_total_requests= ""
	...
	def get_object_id(self) :
		try:
			return super(application_name,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Number of vservers behind CS or any other vserver
	'''
	@property
	...
	def vserver_count(self,vserver_count):
		try :
			if not isinstance(vserver_count,float):
				raise TypeError("vserver_count must be set to float value")
	...
	'''
	@property
	def ssl_cipher_failure_backend(self):
		try:
			return self._ssl_cipher_failure_backend
		except Exception as e :
			raise e
	'''
	SSL cipher failure backend.
	'''
	@ssl_cipher_failure_backend.setter
	def ssl_cipher_failure_backend(self,ssl_cipher_failure_backend):
		try :
			if not isinstance(ssl_cipher_failure_backend,str):
				raise TypeError("ssl_cipher_failure_backend must be set to str value")
			self._ssl_cipher_failure_backend = ssl_cipher_failure_backend
		except Exception as e :
			raise e
	...
	'''
	@property
	def ssl_cipher_failure_frontend(self):
		try:
			return self._ssl_cipher_failure_frontend
		except Exception as e :
			raise e
	'''
	SSL cipher failure frontend.
	'''
	@ssl_cipher_failure_frontend.setter
	def ssl_cipher_failure_frontend(self,ssl_cipher_failure_frontend):
		try :
			if not isinstance(ssl_cipher_failure_frontend,str):
				raise TypeError("ssl_cipher_failure_frontend must be set to str value")
			self._ssl_cipher_failure_frontend = ssl_cipher_failure_frontend
		except Exception as e :
			raise e
	...
	'''
	Total requests to this APP in given sampled timeframe.
	'''
	@property
	...
	'''
	Af Report for Top Application by Request Count Managed by this AF Collector.
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				application_name_obj=application_name()
				response = application_name_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of application_name resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return application_name_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the application_name resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = application_name_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of application_name resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = application_name_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(application_name_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(application_name_responses, response, "application_name_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.application_name_response_array
				i=0
				error = [application_name() for _ in range(len(response))]
				for obj in response :
			...
			response = result.application_name_response_array
			i=0
			application_name_objs = [application_name() for _ in range(len(response))]
			for obj in response :
				for props in obj._application_name:
					result = service.payload_formatter.string_to_bulk_resource(application_name_response,self.__class__.__name__,props)
					application_name_objs[i] = result.application_name
					i=i+1
			return application_name_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(application_name,self).validate()
		except Exception as e :
			raise e
...
'''
class application_name_response(base_response):
	def __init__(self,length=1) :
		self.application_name= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.application_name= [ application_name() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class application_name_responses(base_response):
	def __init__(self,length=1) :
		self.application_name_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.application_name_response_array = [ application_name() for _ in range(length)]
