...
class external_server(base_resource):
	...
	'''
	get If true, external server is configured for secondary server authentication
	'''
	@property
	def is_secondary_external_server(self) :
		try:
			return self._is_secondary_external_server
		except Exception as e :
			raise e
	'''
	set If true, external server is configured for secondary server authentication
	'''
	@is_secondary_external_server.setter
	def is_secondary_external_server(self,is_secondary_external_server):
		try :
			if not isinstance(is_secondary_external_server,bool):
				raise TypeError("is_secondary_external_server must be set to bool value")
			self._is_secondary_external_server = is_secondary_external_server
		except Exception as e :
			raise e
	...
	'''
	get Id is system generated key for all the external servers
	'''
	@property
	def id(self) :
		try:
			...
			raise e
	'''
	set Id is system generated key for all the external servers
	'''
	@id.setter
	def id(self,id):
		try :
	...
	'''
	Use this operation to get the external server information.
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			response=""
			if not resource :
				external_server_obj=external_server()
				response = external_server_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of external_server resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return external_server_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the external_server resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = external_server_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of external_server resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = external_server_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
