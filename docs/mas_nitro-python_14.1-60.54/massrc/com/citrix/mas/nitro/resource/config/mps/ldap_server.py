...
class ldap_server(base_resource):
	...
	'''
	get Configuration Type. Values: 0: IPv4, 1: IPv6, -1: Hostname
	'''
	@property
	...
	'''
	get SSH public key attribute holds the public keys of the user
	'''
	@property
	def ssh_public_key(self) :
		try:
			...
			raise e
	'''
	set SSH public key attribute holds the public keys of the user
	'''
	@ssh_public_key.setter
	def ssh_public_key(self,ssh_public_key):
		try :
			if not isinstance(ssh_public_key,str):
				raise TypeError("ssh_public_key must be set to str value")
	...
	'''
	get Set to false for public key Authentication
	'''
	@property
	def authentication(self) :
		try:
			...
			raise e
	'''
	set Set to false for public key Authentication
	'''
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				ldap_server_obj= ldap_server()
				return cls.perform_operation_bulk_request(service,resource,ldap_server_obj)
		except Exception as e :
			raise e
