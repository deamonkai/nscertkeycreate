...
class login(base_resource):
	...
	_two_factor_authentication_label= ""
	_session_creator= ""
	_auth_with_remote_pubkey= ""
	_ID= ""
	_password= ""
	_session_timeout= ""
	_client_port= ""
	...
	def confirm_new_password(self,confirm_new_password):
		try :
			if not isinstance(confirm_new_password,str):
				raise TypeError("confirm_new_password must be set to str value")
	...
	'''
	@property
	def two_factor_authentication_label(self):
		try:
			return self._two_factor_authentication_label
		except Exception as e :
			raise e
	'''
	Two-factor authentication label
	'''
	@two_factor_authentication_label.setter
	def two_factor_authentication_label(self,two_factor_authentication_label):
		try :
			if not isinstance(two_factor_authentication_label,str):
				raise TypeError("two_factor_authentication_label must be set to str value")
			self._two_factor_authentication_label = two_factor_authentication_label
		except Exception as e :
			raise e
	...
	session_creator
	'''
	@session_creator.setter
	def session_creator(self,session_creator):
		try :
			if not isinstance(session_creator,str):
				raise TypeError("session_creator must be set to str value")
			self._session_creator = session_creator
		except Exception as e :
			raise e

	'''
	Login request to specify if SSH authentication took place with remote public keys for this user, 1 for remotely managed pubkey else 0
	'''
	@property
	def auth_with_remote_pubkey(self):
		try:
			return self._auth_with_remote_pubkey
		except Exception as e :
			raise e
	'''
	Login request to specify if SSH authentication took place with remote public keys for this user, 1 for remotely managed pubkey else 0
	'''
	@auth_with_remote_pubkey.setter
	def auth_with_remote_pubkey(self,auth_with_remote_pubkey):
		try :
			if not isinstance(auth_with_remote_pubkey,int):
				raise TypeError("auth_with_remote_pubkey must be set to int value")
			self._auth_with_remote_pubkey = auth_with_remote_pubkey
		except Exception as e :
			raise e

	'''
	ID of the User that wants to login in cloud
	'''
	@property
	def ID(self):
		try:
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				login_obj= login()
				return cls.perform_operation_bulk_request(service,resource,login_obj)
		except Exception as e :
			raise e
