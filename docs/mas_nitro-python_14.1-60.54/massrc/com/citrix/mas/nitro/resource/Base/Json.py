...
class Json(ipayload_formatter):
    ...
    * @return returns a String
    ...
    def resource_to_object_convert(self, resrc):
        try:
            dict_valid_values={}
            resrc_action_keys = resrc._request_params

            for k,v in resrc.__dict__.items() :
                k=k[1:]
                if(not resrc.is_body_property(k)):
                    continue
                if resrc_action_keys and k not in  resrc_action_keys:
                    continue
    ...
    * @return returns nitro resource object.
    ...
    def string_to_resource(self, responseClass, response_dict, resrc_type ,bulk = "", class_name = None):
        try:
            ...
            op_by_primary_key_name=response.__dict__[resrc_type][0]._op_by_primary_key_name
            ...
            for key in temp_dict.keys() :
                ...
                if key == 'additionalInfo':
                    ...
                    resrc_action_keys = (response.__dict__[key])[0]._response_params
                    if type(temp_dict[key]) is list :
                        for i in range(length) :
                            underscore_dict = {}
                            for each_key in (temp_dict[key][i]).keys() :
                                if resrc_action_keys and each_key not in resrc_action_keys:
                                    continue
                        ...
                        for item in temp_dict[key].keys() :
                            if resrc_action_keys and item not in item:
                                continue
                            (response.__dict__[key])[0].__dict__['_'+item] = (temp_dict[key])[item]
                elif plural_key in response.__dict__ and type(response.__dict__[plural_key]) is list:
                    resrc_action_keys = (response.__dict__[plural_key])[0]._response_params
                    if type(temp_dict[key]) is list :
                        #handling case where response is not initiased to valid length
                        if len(response.__dict__[plural_key]) != len(temp_dict[key]):
                            length = len(temp_dict[key])
                            response = responseClass(length)
                        for i in range(length) :
                            underscore_dict = {}
                            for each_key in (temp_dict[key][i]).keys() :
                                if resrc_action_keys and each_key not in resrc_action_keys:
                                    continue
                                underscore_dict['_'+each_key] = (temp_dict[key][i])[each_key]
                            (response.__dict__[plural_key])[i].__dict__ = underscore_dict
                    for item in temp_dict[key].keys():
                        if resrc_action_keys and item not in resrc_action_keys:
                            continue
                        (response.__dict__[plural_key])[0].__dict__['_' + item] = (temp_dict[key])[item]
                elif key == op_by_primary_key_name and type(response.__dict__[resrc_type]) is list:
                    resrc_action_keys = (response.__dict__[resrc_type])[0]._response_params

                    if type(temp_dict[key]) is list :
                        #handling case where response is not initiased to valid length
                        if len(response.__dict__[resrc_type]) != len(temp_dict[key]):
                            length = len(temp_dict[key])
                            response = responseClass(length)
                        for i in range(length) :
                            underscore_dict = {}
                            for each_key in (temp_dict[key][i]).keys() :
                                if resrc_action_keys and each_key not in resrc_action_keys:
                                    continue
                                underscore_dict['_'+each_key] = (temp_dict[key][i])[each_key]
                            (response.__dict__[resrc_type])[i].__dict__ = underscore_dict
                    else:
                        for item in temp_dict[key].keys():
                            if resrc_action_keys and item not in resrc_action_keys:
                                continue
                            (response.__dict__[resrc_type])[0].__dict__['_' + item] = (temp_dict[key])[item]
                else :
    ...
    * @return returns a String
    ...
    def resource_to_string(self, resrc ,option ="",onerror=""):
        try:
            objtype_str = resrc.__class__.__name__
            result = "{ "
            if option:
                if option.version=="v1":
                    ...
                    result = result + "},"
            #For cases where payload object key is different that resource type example
            if resrc._op_by_primary_key_name:
                objtype_str = resrc._op_by_primary_key_name

            result = result + "\"" + objtype_str + "\":" + self.resource_to_string_convert(resrc) + "}"
            return result
