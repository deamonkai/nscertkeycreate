...
class operating_system(af_generic_api):
	_total_bytes= ""
	...
	def get_object_id(self) :
		try:
			return super(operating_system,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Count of this operating system in given sampled timeframe.
	'''
	@property
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				operating_system_obj=operating_system()
				response = operating_system_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of operating_system resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return operating_system_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the operating_system resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = operating_system_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of operating_system resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = operating_system_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(operating_system_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.operating_system
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(operating_system_responses, response, "operating_system_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.operating_system_response_array
				i=0
				error = [operating_system() for _ in range(len(response))]
				for obj in response :
			...
			response = result.operating_system_response_array
			i=0
			operating_system_objs = [operating_system() for _ in range(len(response))]
			for obj in response :
				for props in obj._operating_system:
					result = service.payload_formatter.string_to_bulk_resource(operating_system_response,self.__class__.__name__,props)
					operating_system_objs[i] = result.operating_system
					i=i+1
			return operating_system_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(operating_system,self).validate()
		except Exception as e :
			raise e
...
'''
class operating_system_response(base_response):
	def __init__(self,length=1) :
		self.operating_system= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.operating_system= [ operating_system() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class operating_system_responses(base_response):
	def __init__(self,length=1) :
		self.operating_system_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.operating_system_response_array = [ operating_system() for _ in range(length)]
