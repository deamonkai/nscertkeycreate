...
'''
Configuration for AF API Geo Location Report table resource
'''

class api_geolocation(af_generic_api):
	_region= ""
	_api_endpoint= ""
	_country= ""
	_longitude= ""
	...
	def get_object_id(self) :
		try:
			return super(api_geolocation,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Total Country in given sampled timeframe.
	'''
	@property
	def total_country(self):
		try:
			return self._total_country
		except Exception as e :
			raise e
	'''
	Total Country in given sampled timeframe.
	'''
	@total_country.setter
	def total_country(self,total_country):
		try :
			if not isinstance(total_country,float):
				raise TypeError("total_country must be set to float value")
			self._total_country = total_country
		except Exception as e :
			raise e
	...
	'''
	Maximum Response Time in sampled timeframe.
	'''
	@property
	...
	'''
	AF API Geo Location Report table..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				api_geolocation_obj=api_geolocation()
				response = api_geolocation_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of api_geolocation resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return api_geolocation_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the api_geolocation resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = api_geolocation_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of api_geolocation resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = api_geolocation_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(api_geolocation_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(api_geolocation_responses, response, "api_geolocation_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.api_geolocation_response_array
				i=0
				error = [api_geolocation() for _ in range(len(response))]
				for obj in response :
			...
			response = result.api_geolocation_response_array
			i=0
			api_geolocation_objs = [api_geolocation() for _ in range(len(response))]
			for obj in response :
				for props in obj._api_geolocation:
					result = service.payload_formatter.string_to_bulk_resource(api_geolocation_response,self.__class__.__name__,props)
					api_geolocation_objs[i] = result.api_geolocation
					i=i+1
			return api_geolocation_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(api_geolocation,self).validate()
		except Exception as e :
			raise e
...
'''
class api_geolocation_response(base_response):
	def __init__(self,length=1) :
		self.api_geolocation= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.api_geolocation= [ api_geolocation() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class api_geolocation_responses(base_response):
	def __init__(self,length=1) :
		self.api_geolocation_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.api_geolocation_response_array = [ api_geolocation() for _ in range(length)]
