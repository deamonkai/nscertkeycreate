...
class swg_domain_summary(base_resource):
	_domain_name= ""
	_exporter_id= ""
	_total_bytes_out= ""
	_operating_system= ""
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				swg_domain_summary_obj=swg_domain_summary()
				response = swg_domain_summary_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of swg_domain_summary resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return swg_domain_summary_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the swg_domain_summary resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = swg_domain_summary_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of swg_domain_summary resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = swg_domain_summary_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(swg_domain_summary_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.swg_domain_summary
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(swg_domain_summary_responses, response, "swg_domain_summary_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.swg_domain_summary_response_array
				i=0
				error = [swg_domain_summary() for _ in range(len(response))]
				for obj in response :
			...
			response = result.swg_domain_summary_response_array
			i=0
			swg_domain_summary_objs = [swg_domain_summary() for _ in range(len(response))]
			for obj in response :
				for props in obj._swg_domain_summary:
					result = service.payload_formatter.string_to_bulk_resource(swg_domain_summary_response,self.__class__.__name__,props)
					swg_domain_summary_objs[i] = result.swg_domain_summary
					i=i+1
			return swg_domain_summary_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(swg_domain_summary,self).validate()
		except Exception as e :
			raise e
...
'''
class swg_domain_summary_response(base_response):
	def __init__(self,length=1) :
		self.swg_domain_summary= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.swg_domain_summary= [ swg_domain_summary() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class swg_domain_summary_responses(base_response):
	def __init__(self,length=1) :
		self.swg_domain_summary_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.swg_domain_summary_response_array = [ swg_domain_summary() for _ in range(length)]
