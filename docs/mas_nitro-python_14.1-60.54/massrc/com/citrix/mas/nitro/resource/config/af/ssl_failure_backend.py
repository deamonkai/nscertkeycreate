...
class ssl_failure_backend(af_generic_api):
	_client_ip_address= ""
	_ctnsappname_b= ""
	___count= ""
	_rpt_sample_time= ""
	...
	def get_object_id(self) :
		try:
			return super(ssl_failure_backend,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Af Report for SSL failure backend..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ssl_failure_backend_obj=ssl_failure_backend()
				response = ssl_failure_backend_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ssl_failure_backend resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ssl_failure_backend_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ssl_failure_backend resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ssl_failure_backend_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ssl_failure_backend resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ssl_failure_backend_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ssl_failure_backend_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.ssl_failure_backend
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ssl_failure_backend_responses, response, "ssl_failure_backend_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ssl_failure_backend_response_array
				i=0
				error = [ssl_failure_backend() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ssl_failure_backend_response_array
			i=0
			ssl_failure_backend_objs = [ssl_failure_backend() for _ in range(len(response))]
			for obj in response :
				for props in obj._ssl_failure_backend:
					result = service.payload_formatter.string_to_bulk_resource(ssl_failure_backend_response,self.__class__.__name__,props)
					ssl_failure_backend_objs[i] = result.ssl_failure_backend
					i=i+1
			return ssl_failure_backend_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ssl_failure_backend,self).validate()
		except Exception as e :
			raise e
...
'''
class ssl_failure_backend_response(base_response):
	def __init__(self,length=1) :
		self.ssl_failure_backend= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ssl_failure_backend= [ ssl_failure_backend() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ssl_failure_backend_responses(base_response):
	def __init__(self,length=1) :
		self.ssl_failure_backend_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ssl_failure_backend_response_array = [ ssl_failure_backend() for _ in range(length)]
