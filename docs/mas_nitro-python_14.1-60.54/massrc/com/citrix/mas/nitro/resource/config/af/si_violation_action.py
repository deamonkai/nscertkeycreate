...
class si_violation_action(af_generic_api):
	___count= ""
	_threat_index= ""
	_total_attacks= ""
	...
	def get_object_id(self) :
		try:
			return super(si_violation_action,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				si_violation_action_obj=si_violation_action()
				response = si_violation_action_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of si_violation_action resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return si_violation_action_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the si_violation_action resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = si_violation_action_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of si_violation_action resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = si_violation_action_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(si_violation_action_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.si_violation_action
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(si_violation_action_responses, response, "si_violation_action_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.si_violation_action_response_array
				i=0
				error = [si_violation_action() for _ in range(len(response))]
				for obj in response :
			...
			response = result.si_violation_action_response_array
			i=0
			si_violation_action_objs = [si_violation_action() for _ in range(len(response))]
			for obj in response :
				for props in obj._si_violation_action:
					result = service.payload_formatter.string_to_bulk_resource(si_violation_action_response,self.__class__.__name__,props)
					si_violation_action_objs[i] = result.si_violation_action
					i=i+1
			return si_violation_action_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(si_violation_action,self).validate()
		except Exception as e :
			raise e
...
'''
class si_violation_action_response(base_response):
	def __init__(self,length=1) :
		self.si_violation_action= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.si_violation_action= [ si_violation_action() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class si_violation_action_responses(base_response):
	def __init__(self,length=1) :
		self.si_violation_action_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.si_violation_action_response_array = [ si_violation_action() for _ in range(length)]
