...
class aaa_server(base_resource):
	...
	'''
	get Log external group info
	'''
	@property
	def log_ext_group_info(self) :
		try:
			return self._log_ext_group_info
		except Exception as e :
			raise e
	'''
	set Log external group info
	'''
	@log_ext_group_info.setter
	def log_ext_group_info(self,log_ext_group_info):
		try :
			if not isinstance(log_ext_group_info,bool):
				raise TypeError("log_ext_group_info must be set to bool value")
			self._log_ext_group_info = log_ext_group_info
		except Exception as e :
			raise e


	'''
	get Two-factor authentication label
	'''
	@property
	def two_factor_label(self) :
		try:
			return self._two_factor_label
		except Exception as e :
			raise e
	'''
	set Two-factor authentication label
	'''
	@two_factor_label.setter
	def two_factor_label(self,two_factor_label):
		try :
			if not isinstance(two_factor_label,str):
				raise TypeError("two_factor_label must be set to str value")
			self._two_factor_label = two_factor_label
		except Exception as e :
			raise e
	...
	'''
	get List of secondary external servers for two-factor authentication
	'''
	@property
	def secondary_external_servers(self) :
		try:
			return self._secondary_external_servers
		except Exception as e :
			raise e
	'''
	set List of secondary external servers for two-factor authentication
	'''
	@secondary_external_servers.setter
	def secondary_external_servers(self,secondary_external_servers) :
		try :
			if not isinstance(secondary_external_servers,list):
				raise TypeError("secondary_external_servers must be set to array of external_server value")
			for item in secondary_external_servers :
				if not isinstance(item,external_server):
					raise TypeError("item must be set to external_server value")
			self._secondary_external_servers = secondary_external_servers
		except Exception as e :
			raise e
	...
	'''
	get List of cascaded external servers for single factor authentication. For two-factor authentication, list of primary external servers
	'''
	@property
	def external_servers(self) :
		try:
			...
			raise e
	'''
	set List of cascaded external servers for single factor authentication. For two-factor authentication, list of primary external servers
	'''
