...
class tacacs_server(base_resource):
	...
	'''
	get The Attribute name for group extraction from the ACS server. If not passed, then groups will not be extracted. No other harm
	'''
	@property
	def group_attr_name(self) :
		try:
			...
			raise e
	'''
	set The Attribute name for group extraction from the ACS server. If not passed, then groups will not be extracted. No other harm
	'''
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				tacacs_server_obj= tacacs_server()
				return cls.perform_operation_bulk_request(service,resource,tacacs_server_obj)
		except Exception as e :
			raise e
