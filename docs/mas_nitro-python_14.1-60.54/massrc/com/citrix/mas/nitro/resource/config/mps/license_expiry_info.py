...
'''
Configuration for NetScaler Console License Expiry Information resource
'''
...
class license_expiry_info(base_resource):
	...
	'''
	get Contract Expiry Days
	'''
	@property
	def contract_expiry(self) :
		try:
			return self._contract_expiry
		except Exception as e :
			raise e
	'''
	set Contract Expiry Days
	'''
	@contract_expiry.setter
	def contract_expiry(self,contract_expiry):
		try :
			if not isinstance(contract_expiry,int):
				raise TypeError("contract_expiry must be set to int value")
			self._contract_expiry = contract_expiry
		except Exception as e :
			raise e

	'''
	Type of license: RetailS etc
	'''
	@property
