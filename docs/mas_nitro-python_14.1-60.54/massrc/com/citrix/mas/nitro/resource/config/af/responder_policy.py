...
'''
Configuration for Responder Policy Hit Counters Report resource
'''

class responder_policy(af_generic_api):
	_policy_name= ""
	_pcp_hits= ""
	_rpt_sample_time= ""
	___count= ""
	...
	def get_object_id(self) :
		try:
			return super(responder_policy,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				responder_policy_obj=responder_policy()
				response = responder_policy_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of responder_policy resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return responder_policy_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the responder_policy resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = responder_policy_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of responder_policy resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = responder_policy_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(responder_policy_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.responder_policy
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(responder_policy_responses, response, "responder_policy_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.responder_policy_response_array
				i=0
				error = [responder_policy() for _ in range(len(response))]
				for obj in response :
			...
			response = result.responder_policy_response_array
			i=0
			responder_policy_objs = [responder_policy() for _ in range(len(response))]
			for obj in response :
				for props in obj._responder_policy:
					result = service.payload_formatter.string_to_bulk_resource(responder_policy_response,self.__class__.__name__,props)
					responder_policy_objs[i] = result.responder_policy
					i=i+1
			return responder_policy_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(responder_policy,self).validate()
		except Exception as e :
			raise e
...
'''
class responder_policy_response(base_response):
	def __init__(self,length=1) :
		self.responder_policy= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.responder_policy= [ responder_policy() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class responder_policy_responses(base_response):
	def __init__(self,length=1) :
		self.responder_policy_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.responder_policy_response_array = [ responder_policy() for _ in range(length)]
