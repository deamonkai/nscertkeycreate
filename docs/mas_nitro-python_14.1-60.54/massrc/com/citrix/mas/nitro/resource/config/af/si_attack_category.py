...
class si_attack_category(af_generic_api):
	_id= ""
	_si_app_unit_name= ""
	_name= ""
	...
	def get_object_id(self) :
		try:
			return super(si_attack_category,self).get_object_id()
		except Exception as e :
			raise e
	...
	def file_component_value(self) :
		try :
			return "si_attack_categorys"
		except Exception as e :
			raise e


	'''
	Id is attack category name
	'''
	@property
	def id(self):
		try:
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				si_attack_category_obj=si_attack_category()
				response = si_attack_category_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of si_attack_category resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return si_attack_category_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the si_attack_category resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = si_attack_category_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of si_attack_category resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = si_attack_category_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(si_attack_category_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.si_attack_category
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(si_attack_category_responses, response, "si_attack_category_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.si_attack_category_response_array
				i=0
				error = [si_attack_category() for _ in range(len(response))]
				for obj in response :
			...
			response = result.si_attack_category_response_array
			i=0
			si_attack_category_objs = [si_attack_category() for _ in range(len(response))]
			for obj in response :
				for props in obj._si_attack_category:
					result = service.payload_formatter.string_to_bulk_resource(si_attack_category_response,self.__class__.__name__,props)
					si_attack_category_objs[i] = result.si_attack_category
					i=i+1
			return si_attack_category_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(si_attack_category,self).validate()
		except Exception as e :
			raise e
...
'''
class si_attack_category_response(base_response):
	def __init__(self,length=1) :
		self.si_attack_category= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.si_attack_category= [ si_attack_category() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class si_attack_category_responses(base_response):
	def __init__(self,length=1) :
		self.si_attack_category_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.si_attack_category_response_array = [ si_attack_category() for _ in range(length)]
