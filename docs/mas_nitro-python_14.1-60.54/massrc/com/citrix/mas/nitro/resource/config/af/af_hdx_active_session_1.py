...
'''
Configuration for AF HDX Session Report table for Level 1 resource
'''

class af_hdx_active_session_1(base_resource):
	_L7_serverside_latency= ""
	_avg_l7lat_breach_server= ""
	_total_bytes= ""
	_is_multi_hop= ""
	_client_version= ""
	_server_ip= ""
	...
	def get_object_type(self) :
		try:
			return "af_hdx_active_session_1"
	...
	def file_component_value(self) :
		try :
			return "af_hdx_active_session_1s"
		except Exception as e :
			raise e
	...
	'''
	get avgL7latBreachServer
	'''
	@property
	def avg_l7lat_breach_server(self) :
		try:
			return self._avg_l7lat_breach_server
		except Exception as e :
			raise e
	'''
	set avgL7latBreachServer
	'''
	@avg_l7lat_breach_server.setter
	def avg_l7lat_breach_server(self,avg_l7lat_breach_server):
		try :
			if not isinstance(avg_l7lat_breach_server,float):
				raise TypeError("avg_l7lat_breach_server must be set to float value")
			self._avg_l7lat_breach_server = avg_l7lat_breach_server
		except Exception as e :
			raise e
	...
	def is_multi_hop(self,is_multi_hop):
		try :
			if not isinstance(is_multi_hop,int):
				raise TypeError("is_multi_hop must be set to int value")
	...
	def vpn_session_id(self) :
		try:
			...
			raise e
	'''
	set VPN Session GUID.
	'''
	@vpn_session_id.setter
	def vpn_session_id(self,vpn_session_id):
		try :
			if not isinstance(vpn_session_id,str):
				raise TypeError("vpn_session_id must be set to str value")
	...
	def is_active(self,is_active):
		try :
			if not isinstance(is_active,int):
				raise TypeError("is_active must be set to int value")
	...
	'''
	get Hop count of this exporter.
	'''
	@property
	def hop_count(self) :
		try:
			...
			raise e
	'''
	set Hop count of this exporter.
	'''
	@hop_count.setter
	def hop_count(self,hop_count):
		try :
			if not isinstance(hop_count,int):
				raise TypeError("hop_count must be set to int value")
	...
	'''
	get Session Reliability ACR Reconnect count
	'''
	@property
	def acr_count(self) :
		try:
			...
			raise e
	'''
	set Session Reliability ACR Reconnect count
	'''
	@acr_count.setter
	...
	def server_jitter(self,server_jitter):
		try :
			if not isinstance(server_jitter,int):
				raise TypeError("server_jitter must be set to int value")
	...
	'''
	get Session Reliability Reconnect count
	'''
	@property
	def sr_reconnect_count(self) :
		try:
			...
			raise e
	'''
	set Session Reliability Reconnect count
	'''
	...
	'''
	get Avg L7 Latency Breach
	'''
	@property
	def avg_l7lat_breach_client(self) :
		try:
			return self._avg_l7lat_breach_client
		except Exception as e :
			raise e
	'''
	set Avg L7 Latency Breach
	'''
	@avg_l7lat_breach_client.setter
	def avg_l7lat_breach_client(self,avg_l7lat_breach_client):
		try :
			if not isinstance(avg_l7lat_breach_client,float):
				raise TypeError("avg_l7lat_breach_client must be set to float value")
			self._avg_l7lat_breach_client = avg_l7lat_breach_client
		except Exception as e :
			raise e
	...
	'''
	get ICA Session setup time in local timezone.
	'''
	@property
	def session_setup_time_local(self) :
		try:
			...
			raise e
	'''
	set ICA Session setup time in local timezone.
	'''
	@session_setup_time_local.setter
	def session_setup_time_local(self,session_setup_time_local):
		try :
			if not isinstance(session_setup_time_local,long):
				raise TypeError("session_setup_time_local must be set to long value")
	...
	def client_jitter(self,client_jitter):
		try :
			if not isinstance(client_jitter,int):
				raise TypeError("client_jitter must be set to int value")
	...
	def server_delay(self,server_delay):
		try :
			if not isinstance(server_delay,float):
				raise TypeError("server_delay must be set to float value")
	...
	def active_app_count(self) :
		try:
			...
			raise e
	'''
	set Active App Count.
	'''
	@active_app_count.setter
	def active_app_count(self,active_app_count):
		try :
			if not isinstance(active_app_count,int):
				raise TypeError("active_app_count must be set to int value")
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				af_hdx_active_session_1_obj=af_hdx_active_session_1()
				response = af_hdx_active_session_1_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of af_hdx_active_session_1 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return af_hdx_active_session_1_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the af_hdx_active_session_1 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = af_hdx_active_session_1_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of af_hdx_active_session_1 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = af_hdx_active_session_1_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_hdx_active_session_1_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.af_hdx_active_session_1
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_hdx_active_session_1_responses, response, "af_hdx_active_session_1_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.af_hdx_active_session_1_response_array
				i=0
				error = [af_hdx_active_session_1() for _ in range(len(response))]
				for obj in response :
			...
			response = result.af_hdx_active_session_1_response_array
			i=0
			af_hdx_active_session_1_objs = [af_hdx_active_session_1() for _ in range(len(response))]
			for obj in response :
				for props in obj._af_hdx_active_session_1:
					result = service.payload_formatter.string_to_bulk_resource(af_hdx_active_session_1_response,self.__class__.__name__,props)
					af_hdx_active_session_1_objs[i] = result.af_hdx_active_session_1
					i=i+1
			return af_hdx_active_session_1_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(af_hdx_active_session_1,self).validate()
		except Exception as e :
			raise e
...
'''
class af_hdx_active_session_1_response(base_response):
	def __init__(self,length=1) :
		self.af_hdx_active_session_1= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.af_hdx_active_session_1= [ af_hdx_active_session_1() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class af_hdx_active_session_1_responses(base_response):
	def __init__(self,length=1) :
		self.af_hdx_active_session_1_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.af_hdx_active_session_1_response_array = [ af_hdx_active_session_1() for _ in range(length)]
