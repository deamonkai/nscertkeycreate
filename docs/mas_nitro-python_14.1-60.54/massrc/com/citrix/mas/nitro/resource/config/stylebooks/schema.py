...
class schema(base_resource):
	...
	def get_namespace_url(self) :
		try:
			namespace_url = ""
			object_types_list = ["stylebooks","schema"]
	...
	'''
	get Parameters of the StyleBook
	'''
	@property
	def parameters(self) :
		try:
			...
			raise e
	'''
	set Parameters of the StyleBook
	'''
	@parameters.setter
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				schema_obj=schema()
				response = schema_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of schema resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return schema_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the schema resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = schema_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of schema resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = schema_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(schema_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(schema_responses, response, "schema_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.schema_response_array
				i=0
				error = [schema() for _ in range(len(response))]
				for obj in response :
			...
			response = result.schema_response_array
			i=0
			schema_objs = [schema() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_schema'):
					for props in obj._schema:
						result = service.payload_formatter.string_to_bulk_resource(schema_response,self.__class__.__name__,props)
						schema_objs[i] = result.schema
						i=i+1
			return schema_objs
		except Exception as e :
			raise e
...
'''
class schema_response(base_response):
	def __init__(self,length=1) :
		self.schema= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.schema= [ schema() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class schema_responses(base_response):
	def __init__(self,length=1) :
		self.schema_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.schema_response_array = [ schema() for _ in range(length)]
