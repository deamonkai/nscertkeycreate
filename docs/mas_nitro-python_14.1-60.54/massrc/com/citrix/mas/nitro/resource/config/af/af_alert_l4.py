...
'''
Configuration for Threshold breach information for Level 4 resource
'''

class af_alert_l4(base_resource):
	_rpt_sample_time= ""
	_threshold_name= ""
	_violation_count_l4= ""
	_entity_type= ""
	...
	'''
	get No of times violation happens for particular entity for l4 threshold configuration
	'''
	@property
	def violation_count_l4(self) :
		try:
			return self._violation_count_l4
		except Exception as e :
			raise e
	'''
	set No of times violation happens for particular entity for l4 threshold configuration
	'''
	@violation_count_l4.setter
	def violation_count_l4(self,violation_count_l4):
		try :
			if not isinstance(violation_count_l4,long):
				raise TypeError("violation_count_l4 must be set to long value")
			self._violation_count_l4 = violation_count_l4
		except Exception as e :
			raise e
	...
	'''
	get No of times violation happens for particular entity for l5 threshold configuration
	'''
	@property
	def violation_count_l5(self) :
		try:
			return self._violation_count_l5
		except Exception as e :
			raise e
	'''
	set No of times violation happens for particular entity for l5 threshold configuration
	'''
	@violation_count_l5.setter
	def violation_count_l5(self,violation_count_l5):
		try :
			if not isinstance(violation_count_l5,long):
				raise TypeError("violation_count_l5 must be set to long value")
			self._violation_count_l5 = violation_count_l5
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				af_alert_l4_obj=af_alert_l4()
				response = af_alert_l4_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of af_alert_l4 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return af_alert_l4_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the af_alert_l4 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = af_alert_l4_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of af_alert_l4 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = af_alert_l4_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_alert_l4_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_alert_l4_responses, response, "af_alert_l4_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.af_alert_l4_response_array
				i=0
				error = [af_alert_l4() for _ in range(len(response))]
				for obj in response :
			...
			response = result.af_alert_l4_response_array
			i=0
			af_alert_l4_objs = [af_alert_l4() for _ in range(len(response))]
			for obj in response :
				for props in obj._af_alert_l4:
					result = service.payload_formatter.string_to_bulk_resource(af_alert_l4_response,self.__class__.__name__,props)
					af_alert_l4_objs[i] = result.af_alert_l4
					i=i+1
			return af_alert_l4_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(af_alert_l4,self).validate()
		except Exception as e :
			raise e
...
'''
class af_alert_l4_response(base_response):
	def __init__(self,length=1) :
		self.af_alert_l4= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.af_alert_l4= [ af_alert_l4() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class af_alert_l4_responses(base_response):
	def __init__(self,length=1) :
		self.af_alert_l4_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.af_alert_l4_response_array = [ af_alert_l4() for _ in range(length)]
