...
class backup_component(base_resource):
	...
	'''
	get (32/64 bit) information for backup component version
	'''
	@property
	def component_version_bit(self) :
		try:
			return self._component_version_bit
		except Exception as e :
			raise e
	'''
	set (32/64 bit) information for backup component version
	'''
	@component_version_bit.setter
	def component_version_bit(self,component_version_bit):
		try :
			if not isinstance(component_version_bit,str):
				raise TypeError("component_version_bit must be set to str value")
			self._component_version_bit = component_version_bit
		except Exception as e :
			raise e


	'''
	get Backup component VM Name
	'''
	@property
	def component_hostname(self) :
		try:
			return self._component_hostname
		except Exception as e :
			raise e
	'''
	set Backup component VM Name
	'''
	@component_hostname.setter
	def component_hostname(self,component_hostname):
		try :
			if not isinstance(component_hostname,str):
				raise TypeError("component_hostname must be set to str value")
			self._component_hostname = component_hostname
		except Exception as e :
			raise e
