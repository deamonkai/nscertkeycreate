...
'''
Configuration for AF API Instance Performance Dashboard Report table resource
'''

class api_instance_perf_dashboard(af_generic_api):
	...
	def get_object_type(self) :
		try:
			return "api_instance_perf_dashboard"
	...
	def get_object_id(self) :
		try:
			return super(api_instance_perf_dashboard,self).get_object_id()
		except Exception as e :
			raise e
	...
	def file_component_value(self) :
		try :
			return "api_instance_perf_dashboards"
		except Exception as e :
			raise e
	...
	'''
	AF API Instance Performance Dashboard Report table..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				api_instance_perf_dashboard_obj=api_instance_perf_dashboard()
				response = api_instance_perf_dashboard_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of api_instance_perf_dashboard resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return api_instance_perf_dashboard_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the api_instance_perf_dashboard resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = api_instance_perf_dashboard_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of api_instance_perf_dashboard resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = api_instance_perf_dashboard_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(api_instance_perf_dashboard_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.api_instance_perf_dashboard
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(api_instance_perf_dashboard_responses, response, "api_instance_perf_dashboard_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.api_instance_perf_dashboard_response_array
				i=0
				error = [api_instance_perf_dashboard() for _ in range(len(response))]
				for obj in response :
			...
			response = result.api_instance_perf_dashboard_response_array
			i=0
			api_instance_perf_dashboard_objs = [api_instance_perf_dashboard() for _ in range(len(response))]
			for obj in response :
				for props in obj._api_instance_perf_dashboard:
					result = service.payload_formatter.string_to_bulk_resource(api_instance_perf_dashboard_response,self.__class__.__name__,props)
					api_instance_perf_dashboard_objs[i] = result.api_instance_perf_dashboard
					i=i+1
			return api_instance_perf_dashboard_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(api_instance_perf_dashboard,self).validate()
		except Exception as e :
			raise e
...
'''
class api_instance_perf_dashboard_response(base_response):
	def __init__(self,length=1) :
		self.api_instance_perf_dashboard= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.api_instance_perf_dashboard= [ api_instance_perf_dashboard() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class api_instance_perf_dashboard_responses(base_response):
	def __init__(self,length=1) :
		self.api_instance_perf_dashboard_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.api_instance_perf_dashboard_response_array = [ api_instance_perf_dashboard() for _ in range(length)]
