...
class device_backup(base_resource):
	...
	def backup_filesize(self,backup_filesize):
		try :
			if not isinstance(backup_filesize,long):
				raise TypeError("backup_filesize must be set to long value")
	...
	'''
	get True if wallet key encrypted for service deployment
	'''
	@property
	def wallet_key_encrypted(self) :
		try:
			return self._wallet_key_encrypted
		except Exception as e :
			raise e
	'''
	set True if wallet key encrypted for service deployment
	'''
	@wallet_key_encrypted.setter
	def wallet_key_encrypted(self,wallet_key_encrypted):
		try :
			if not isinstance(wallet_key_encrypted,bool):
				raise TypeError("wallet_key_encrypted must be set to bool value")
			self._wallet_key_encrypted = wallet_key_encrypted
		except Exception as e :
			raise e
