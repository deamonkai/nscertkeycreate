...
class ica_device(af_generic_api):
	_id= ""
	_app_launch_count= ""
	_app_count= ""
	_name= ""
	...
	def get_object_id(self) :
		try:
			return super(ica_device,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ica_device_obj=ica_device()
				response = ica_device_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ica_device resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ica_device_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ica_device resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ica_device_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ica_device resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ica_device_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ica_device_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ica_device_responses, response, "ica_device_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ica_device_response_array
				i=0
				error = [ica_device() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ica_device_response_array
			i=0
			ica_device_objs = [ica_device() for _ in range(len(response))]
			for obj in response :
				for props in obj._ica_device:
					result = service.payload_formatter.string_to_bulk_resource(ica_device_response,self.__class__.__name__,props)
					ica_device_objs[i] = result.ica_device
					i=i+1
			return ica_device_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ica_device,self).validate()
		except Exception as e :
			raise e
...
'''
class ica_device_response(base_response):
	def __init__(self,length=1) :
		self.ica_device= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ica_device= [ ica_device() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ica_device_responses(base_response):
	def __init__(self,length=1) :
		self.ica_device_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ica_device_response_array = [ ica_device() for _ in range(length)]
