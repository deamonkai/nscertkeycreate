...
class http_req_method(af_generic_api):
	_ic_reval= ""
	_server_ip_address= ""
	...
	def get_object_id(self) :
		try:
			return super(http_req_method,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Get all Af Report for HTTP Methods..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				http_req_method_obj=http_req_method()
				response = http_req_method_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of http_req_method resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return http_req_method_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the http_req_method resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = http_req_method_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of http_req_method resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = http_req_method_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(http_req_method_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(http_req_method_responses, response, "http_req_method_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.http_req_method_response_array
				i=0
				error = [http_req_method() for _ in range(len(response))]
				for obj in response :
			...
			response = result.http_req_method_response_array
			i=0
			http_req_method_objs = [http_req_method() for _ in range(len(response))]
			for obj in response :
				for props in obj._http_req_method:
					result = service.payload_formatter.string_to_bulk_resource(http_req_method_response,self.__class__.__name__,props)
					http_req_method_objs[i] = result.http_req_method
					i=i+1
			return http_req_method_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(http_req_method,self).validate()
		except Exception as e :
			raise e
...
'''
class http_req_method_response(base_response):
	def __init__(self,length=1) :
		self.http_req_method= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.http_req_method= [ http_req_method() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class http_req_method_responses(base_response):
	def __init__(self,length=1) :
		self.http_req_method_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.http_req_method_response_array = [ http_req_method() for _ in range(length)]
