...
class api_endpoint_discovery(af_generic_api):
	_api_endpoint= ""
	___count= ""
	_min_query_param= ""
	_max_query_param= ""
	...
	def get_object_id(self) :
		try:
			return super(api_endpoint_discovery,self).get_object_id()
		except Exception as e :
			raise e
	...
	def file_component_value(self) :
		try :
			return "api_endpoint_discoverys"
		except Exception as e :
			raise e
	...
	def status_reason(self,status_reason):
		try :
			if not isinstance(status_reason,str):
				raise TypeError("status_reason must be set to str value")
	...
	def is_auth(self,is_auth):
		try :
			if not isinstance(is_auth,float):
				raise TypeError("is_auth must be set to float value")
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				api_endpoint_discovery_obj=api_endpoint_discovery()
				response = api_endpoint_discovery_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of api_endpoint_discovery resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return api_endpoint_discovery_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the api_endpoint_discovery resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = api_endpoint_discovery_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of api_endpoint_discovery resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = api_endpoint_discovery_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(api_endpoint_discovery_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.api_endpoint_discovery
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(api_endpoint_discovery_responses, response, "api_endpoint_discovery_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.api_endpoint_discovery_response_array
				i=0
				error = [api_endpoint_discovery() for _ in range(len(response))]
				for obj in response :
			...
			response = result.api_endpoint_discovery_response_array
			i=0
			api_endpoint_discovery_objs = [api_endpoint_discovery() for _ in range(len(response))]
			for obj in response :
				for props in obj._api_endpoint_discovery:
					result = service.payload_formatter.string_to_bulk_resource(api_endpoint_discovery_response,self.__class__.__name__,props)
					api_endpoint_discovery_objs[i] = result.api_endpoint_discovery
					i=i+1
			return api_endpoint_discovery_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(api_endpoint_discovery,self).validate()
		except Exception as e :
			raise e
...
'''
class api_endpoint_discovery_response(base_response):
	def __init__(self,length=1) :
		self.api_endpoint_discovery= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.api_endpoint_discovery= [ api_endpoint_discovery() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class api_endpoint_discovery_responses(base_response):
	def __init__(self,length=1) :
		self.api_endpoint_discovery_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.api_endpoint_discovery_response_array = [ api_endpoint_discovery() for _ in range(length)]
