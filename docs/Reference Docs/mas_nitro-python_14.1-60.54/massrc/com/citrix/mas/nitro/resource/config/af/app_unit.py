...
class app_unit(af_generic_api):
	...
	def get_object_id(self) :
		try:
			return super(app_unit,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Af Report for Top APP by Request Count Managed by this AF Collector.
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				app_unit_obj=app_unit()
				response = app_unit_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of app_unit resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return app_unit_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the app_unit resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = app_unit_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of app_unit resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = app_unit_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(app_unit_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(app_unit_responses, response, "app_unit_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.app_unit_response_array
				i=0
				error = [app_unit() for _ in range(len(response))]
				for obj in response :
			...
			response = result.app_unit_response_array
			i=0
			app_unit_objs = [app_unit() for _ in range(len(response))]
			for obj in response :
				for props in obj._app_unit:
					result = service.payload_formatter.string_to_bulk_resource(app_unit_response,self.__class__.__name__,props)
					app_unit_objs[i] = result.app_unit
					i=i+1
			return app_unit_objs
		except Exception as e :
			raise e
...
'''
class app_unit_response(base_response):
	def __init__(self,length=1) :
		self.app_unit= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.app_unit= [ app_unit() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class app_unit_responses(base_response):
	def __init__(self,length=1) :
		self.app_unit_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.app_unit_response_array = [ app_unit() for _ in range(length)]
