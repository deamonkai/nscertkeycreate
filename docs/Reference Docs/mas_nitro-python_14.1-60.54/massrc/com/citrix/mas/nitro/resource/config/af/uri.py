...
class uri(af_generic_api):
	...
	def get_object_id(self) :
		try:
			return super(uri,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	@dom_count.setter
	def dom_count(self,dom_count):
		try :
			if not isinstance(dom_count,float):
				raise TypeError("dom_count must be set to float value")
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				uri_obj=uri()
				response = uri_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of uri resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return uri_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the uri resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = uri_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of uri resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = uri_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(uri_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(uri_responses, response, "uri_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.uri_response_array
				i=0
				error = [uri() for _ in range(len(response))]
				for obj in response :
			...
			response = result.uri_response_array
			i=0
			uri_objs = [uri() for _ in range(len(response))]
			for obj in response :
				for props in obj._uri:
					result = service.payload_formatter.string_to_bulk_resource(uri_response,self.__class__.__name__,props)
					uri_objs[i] = result.uri
					i=i+1
			return uri_objs
		except Exception as e :
			raise e
...
'''
class uri_response(base_response):
	def __init__(self,length=1) :
		self.uri= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.uri= [ uri() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class uri_responses(base_response):
	def __init__(self,length=1) :
		self.uri_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.uri_response_array = [ uri() for _ in range(length)]
