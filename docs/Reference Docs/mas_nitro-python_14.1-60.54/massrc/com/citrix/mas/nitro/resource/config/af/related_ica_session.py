...
class related_ica_session(af_generic_api):
	_sr_reconnect_count= ""
	_ica_user_name= ""
	_user_type= ""
	_serverside_cb= ""
	...
	def get_object_id(self) :
		try:
			return super(related_ica_session,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				related_ica_session_obj=related_ica_session()
				response = related_ica_session_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of related_ica_session resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return related_ica_session_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the related_ica_session resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = related_ica_session_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of related_ica_session resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = related_ica_session_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(related_ica_session_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.related_ica_session
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(related_ica_session_responses, response, "related_ica_session_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.related_ica_session_response_array
				i=0
				error = [related_ica_session() for _ in range(len(response))]
				for obj in response :
			...
			response = result.related_ica_session_response_array
			i=0
			related_ica_session_objs = [related_ica_session() for _ in range(len(response))]
			for obj in response :
				for props in obj._related_ica_session:
					result = service.payload_formatter.string_to_bulk_resource(related_ica_session_response,self.__class__.__name__,props)
					related_ica_session_objs[i] = result.related_ica_session
					i=i+1
			return related_ica_session_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(related_ica_session,self).validate()
		except Exception as e :
			raise e
...
'''
class related_ica_session_response(base_response):
	def __init__(self,length=1) :
		self.related_ica_session= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.related_ica_session= [ related_ica_session() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class related_ica_session_responses(base_response):
	def __init__(self,length=1) :
		self.related_ica_session_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.related_ica_session_response_array = [ related_ica_session() for _ in range(length)]
