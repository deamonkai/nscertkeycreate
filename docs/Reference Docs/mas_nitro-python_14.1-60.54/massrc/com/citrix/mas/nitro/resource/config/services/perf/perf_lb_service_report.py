...
'''
Configuration for LBService resource
'''

class perf_lb_service_report(base_resource):
	_name= ""
	_curclntconnections= ""
	_cursrvrconnections= ""
	_device_ip_address= ""
	...
	def file_component_value(self) :
		try :
			return "perf_lb_service_reports"
		except Exception as e :
			raise e
	...
	'''
	Use this operation to get LB Service Information.
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			response=""
			if not resource :
				perf_lb_service_report_obj=perf_lb_service_report()
				response = perf_lb_service_report_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of perf_lb_service_report resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return perf_lb_service_report_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the perf_lb_service_report resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = perf_lb_service_report_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of perf_lb_service_report resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = perf_lb_service_report_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(perf_lb_service_report_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.perf_lb_service_report
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(perf_lb_service_report_responses, response, "perf_lb_service_report_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.perf_lb_service_report_response_array
				i=0
				error = [perf_lb_service_report() for _ in range(len(response))]
				for obj in response :
			...
			response = result.perf_lb_service_report_response_array
			i=0
			perf_lb_service_report_objs = [perf_lb_service_report() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_perf_lb_service_report'):
					for props in obj._perf_lb_service_report:
						result = service.payload_formatter.string_to_bulk_resource(perf_lb_service_report_response,self.__class__.__name__,props)
						perf_lb_service_report_objs[i] = result.perf_lb_service_report
						i=i+1
			return perf_lb_service_report_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(perf_lb_service_report,self).validate()
		except Exception as e :
			raise e
...
'''
class perf_lb_service_report_response(base_response):
	def __init__(self,length=1) :
		self.perf_lb_service_report= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.perf_lb_service_report= [ perf_lb_service_report() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class perf_lb_service_report_responses(base_response):
	def __init__(self,length=1) :
		self.perf_lb_service_report_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.perf_lb_service_report_response_array = [ perf_lb_service_report() for _ in range(length)]
