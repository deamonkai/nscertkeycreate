...
class jazz_license(base_resource):
	...
	'''
	get Use pooled saas license flag
	'''
	@property
	def is_saas(self) :
		try:
			return self._is_saas
		except Exception as e :
			raise e
	'''
	set Use pooled saas license flag
	'''
	@is_saas.setter
	def is_saas(self,is_saas):
		try :
			if not isinstance(is_saas,bool):
				raise TypeError("is_saas must be set to bool value")
	...
	def fetch(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				res = resource.perform_operation(service,"fetch")
				return res
		except Exception as e :
			raise e
