...
'''
Configuration for AF Bot Attack events table for Level 2 resource
'''

class af_bot_attack_events_l2(base_resource):
	_attack_time= ""
	_bot_signature_category= ""
	_region_code= ""
	_rpt_sample_time= ""
	_bot_true_client_ipv6_address= ""
	_bot_category= ""
	...
	def get_object_type(self) :
		try:
			return "af_bot_attack_events_l2"
	...
	def file_component_value(self) :
		try :
			return "af_bot_attack_events_l2s"
		except Exception as e :
			raise e
	...
	'''
	get Field to store the true client ipv6 address
	'''
	@property
	def bot_true_client_ipv6_address(self) :
		try:
			return self._bot_true_client_ipv6_address
		except Exception as e :
			raise e
	'''
	set Field to store the true client ipv6 address
	'''
	@bot_true_client_ipv6_address.setter
	def bot_true_client_ipv6_address(self,bot_true_client_ipv6_address):
		try :
			if not isinstance(bot_true_client_ipv6_address,str):
				raise TypeError("bot_true_client_ipv6_address must be set to str value")
			self._bot_true_client_ipv6_address = bot_true_client_ipv6_address
		except Exception as e :
			raise e
	...
	def signature_id(self) :
		try:
			...
			raise e
	'''
	set Bot Signature ID.
	'''
	@signature_id.setter
	def signature_id(self,signature_id):
		try :
			if not isinstance(signature_id,int):
				raise TypeError("signature_id must be set to int value")
	...
	def bot_true_client_ip(self) :
		try:
			...
			raise e
	'''
	set Bot True Client IP
	'''
	@bot_true_client_ip.setter
	def bot_true_client_ip(self,bot_true_client_ip):
		try :
			if not isinstance(bot_true_client_ip,long):
				raise TypeError("bot_true_client_ip must be set to long value")
	...
	def action_type(self,action_type):
		try :
			if not isinstance(action_type,int):
				raise TypeError("action_type must be set to int value")
	...
	'''
	Af Report for Bot Attack events..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				af_bot_attack_events_l2_obj=af_bot_attack_events_l2()
				response = af_bot_attack_events_l2_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of af_bot_attack_events_l2 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return af_bot_attack_events_l2_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the af_bot_attack_events_l2 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = af_bot_attack_events_l2_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of af_bot_attack_events_l2 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = af_bot_attack_events_l2_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_bot_attack_events_l2_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.af_bot_attack_events_l2
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_bot_attack_events_l2_responses, response, "af_bot_attack_events_l2_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.af_bot_attack_events_l2_response_array
				i=0
				error = [af_bot_attack_events_l2() for _ in range(len(response))]
				for obj in response :
			...
			response = result.af_bot_attack_events_l2_response_array
			i=0
			af_bot_attack_events_l2_objs = [af_bot_attack_events_l2() for _ in range(len(response))]
			for obj in response :
				for props in obj._af_bot_attack_events_l2:
					result = service.payload_formatter.string_to_bulk_resource(af_bot_attack_events_l2_response,self.__class__.__name__,props)
					af_bot_attack_events_l2_objs[i] = result.af_bot_attack_events_l2
					i=i+1
			return af_bot_attack_events_l2_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(af_bot_attack_events_l2,self).validate()
		except Exception as e :
			raise e
...
'''
class af_bot_attack_events_l2_response(base_response):
	def __init__(self,length=1) :
		self.af_bot_attack_events_l2= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.af_bot_attack_events_l2= [ af_bot_attack_events_l2() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class af_bot_attack_events_l2_responses(base_response):
	def __init__(self,length=1) :
		self.af_bot_attack_events_l2_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.af_bot_attack_events_l2_response_array = [ af_bot_attack_events_l2() for _ in range(length)]
