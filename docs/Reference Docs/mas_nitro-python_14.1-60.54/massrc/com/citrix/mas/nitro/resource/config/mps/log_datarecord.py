...
class log_datarecord(base_resource):
	...
	'''
	get Enable NetScaler gateway insight data record logging value
	'''
	@property
	def vpn(self) :
		try:
			...
			raise e
	'''
	set Enable NetScaler gateway insight data record logging value
	'''
	@vpn.setter
	def vpn(self,vpn):
		try :
			if not isinstance(vpn,bool):
				raise TypeError("vpn must be set to bool value")
			self._vpn = vpn
		except Exception as e :
			raise e
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				log_datarecord_obj= log_datarecord()
				return cls.perform_operation_bulk_request(service,resource,log_datarecord_obj)
		except Exception as e :
			raise e
