...
class ica_session_inactive(af_generic_api):
	_client_type= ""
	_client_tx_bytes= ""
	_server_latency= ""
	_client_srtt= ""
	...
	def get_object_id(self) :
		try:
			return super(ica_session_inactive,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Clinet to Server NetScaler delay.
	'''
	@property
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ica_session_inactive_obj=ica_session_inactive()
				response = ica_session_inactive_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ica_session_inactive resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ica_session_inactive_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ica_session_inactive resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ica_session_inactive_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ica_session_inactive resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ica_session_inactive_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ica_session_inactive_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.ica_session_inactive
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ica_session_inactive_responses, response, "ica_session_inactive_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ica_session_inactive_response_array
				i=0
				error = [ica_session_inactive() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ica_session_inactive_response_array
			i=0
			ica_session_inactive_objs = [ica_session_inactive() for _ in range(len(response))]
			for obj in response :
				for props in obj._ica_session_inactive:
					result = service.payload_formatter.string_to_bulk_resource(ica_session_inactive_response,self.__class__.__name__,props)
					ica_session_inactive_objs[i] = result.ica_session_inactive
					i=i+1
			return ica_session_inactive_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ica_session_inactive,self).validate()
		except Exception as e :
			raise e
...
'''
class ica_session_inactive_response(base_response):
	def __init__(self,length=1) :
		self.ica_session_inactive= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ica_session_inactive= [ ica_session_inactive() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ica_session_inactive_responses(base_response):
	def __init__(self,length=1) :
		self.ica_session_inactive_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ica_session_inactive_response_array = [ ica_session_inactive() for _ in range(length)]
