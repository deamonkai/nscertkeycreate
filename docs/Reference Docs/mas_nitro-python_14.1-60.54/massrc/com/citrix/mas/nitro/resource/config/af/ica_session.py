...
class ica_session(af_generic_api):
	_up_time= ""
	_rpt_sample_time= ""
	...
	def get_object_id(self) :
		try:
			return super(ica_session,self).get_object_id()
		except Exception as e :
			raise e
	...
	'''
	Session Reliability ACR Reconnect count
	'''
	@property
	def acr_count(self):
		try:
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ica_session_obj=ica_session()
				response = ica_session_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ica_session resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ica_session_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ica_session resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ica_session_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ica_session resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ica_session_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ica_session_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ica_session_responses, response, "ica_session_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ica_session_response_array
				i=0
				error = [ica_session() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ica_session_response_array
			i=0
			ica_session_objs = [ica_session() for _ in range(len(response))]
			for obj in response :
				for props in obj._ica_session:
					result = service.payload_formatter.string_to_bulk_resource(ica_session_response,self.__class__.__name__,props)
					ica_session_objs[i] = result.ica_session
					i=i+1
			return ica_session_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ica_session,self).validate()
		except Exception as e :
			raise e
...
'''
class ica_session_response(base_response):
	def __init__(self,length=1) :
		self.ica_session= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ica_session= [ ica_session() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ica_session_responses(base_response):
	def __init__(self,length=1) :
		self.ica_session_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ica_session_response_array = [ ica_session() for _ in range(length)]
