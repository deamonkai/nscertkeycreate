...
class config_variable(base_resource):
	...
	'''
	get Parent name of config variable
	'''
	@property
	def parent_name(self) :
		try:
			...
			raise e
	'''
	set Parent name of config variable
	'''
	@parent_name.setter
	...
	'''
	get Id is system generated key for all config variable
	'''
	@property
	def id(self) :
		try:
			...
			raise e
	'''
	set Id is system generated key for all config variable
	'''
	@id.setter
	def id(self,id):
		try :
	...
	'''
	get Parent Id of config variable
	'''
	@property
	def parent_id(self) :
		try:
			...
			raise e
	'''
	set Parent Id of config variable
	'''
	@parent_id.setter
	...
	'''
	get Comma separated list of possible values of variable
	'''
	@property
	def values_enum_db(self) :
		try:
			...
			raise e
	'''
	set Comma separated list of possible values of variable
	'''
	...
	'''
	get Secure Value of the Variable to be used for all device ips/groups
	'''
	@property
	def valuex(self) :
		try:
			...
			raise e
	'''
	set Secure Value of the Variable to be used for all device ips/groups
	'''
	@valuex.setter
	...
	'''
	get Possible Values values for choice type variable
	'''
	@property
	def values_enum(self) :
		try:
			...
			raise e
	'''
	set Possible Values values for choice type variable
	'''
	@values_enum.setter
	...
	'''
	get Type of Variable Text Field, Numeric Text Field, Choice, File, IP Addrss Field, Password Field
	'''
	@property
	def type(self) :
		try:
			...
			raise e
	'''
	set Type of Variable Text Field, Numeric Text Field, Choice, File, IP Addrss Field, Password Field
	'''
	@type.setter
	def type(self,type):
		try :
	...
	'''
	Use this operation to get config variable..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				...
				response = config_variable_obj.get_resources(client,option_)
			else:
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				config_variable_obj= config_variable()
				return cls.perform_operation_bulk_request(service,resource,config_variable_obj)
		except Exception as e :
			raise e

	'''
	Use this API to fetch filtered set of config_variable resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return config_variable_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the config_variable resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = config_variable_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of config_variable resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = config_variable_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
