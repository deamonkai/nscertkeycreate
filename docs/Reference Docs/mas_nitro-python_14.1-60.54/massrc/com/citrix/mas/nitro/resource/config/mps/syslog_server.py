...
class syslog_server(base_resource):
	...
	'''
	get Events with these log levels pertaining to the shell facilities(local6 & local7) will be sent to this syslog server
	'''
	@property
	def shell_log_levels(self) :
		try:
			return self._shell_log_levels
		except Exception as e :
			raise e
	'''
	set Events with these log levels pertaining to the shell facilities(local6 & local7) will be sent to this syslog server
	'''
	@shell_log_levels.setter
	def shell_log_levels(self,shell_log_levels) :
		try :
			if not isinstance(shell_log_levels,list):
				raise TypeError("shell_log_levels must be set to array of str value")
			for item in shell_log_levels :
				if not isinstance(item,str):
					raise TypeError("item must be set to str value")
			self._shell_log_levels = shell_log_levels
		except Exception as e :
			raise e
	...
	'''
	get Toggle to specify whether event logs pertaining to the shell facilities(local6 & local7) shall be sent to this syslog server
	'''
	@property
	def enable_shell_logging(self) :
		try:
			return self._enable_shell_logging
		except Exception as e :
			raise e
	'''
	set Toggle to specify whether event logs pertaining to the shell facilities(local6 & local7) shall be sent to this syslog server
	'''
	@enable_shell_logging.setter
	def enable_shell_logging(self,enable_shell_logging):
		try :
			if not isinstance(enable_shell_logging,bool):
				raise TypeError("enable_shell_logging must be set to bool value")
			self._enable_shell_logging = enable_shell_logging
		except Exception as e :
			raise e
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				return cls.perform_operation_bulk_request(service,resource,syslog_server_obj)
		except Exception as e :
			raise e
