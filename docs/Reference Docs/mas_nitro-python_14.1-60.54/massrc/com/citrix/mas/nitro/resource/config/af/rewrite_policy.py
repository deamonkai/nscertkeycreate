...
'''
Configuration for Rewrite Policy Hit Counters Report resource
'''

class rewrite_policy(af_generic_api):
	___count= ""
	_rpt_sample_time= ""
	_ns_ip_address= ""
	_pcp_hits= ""
	...
	def get_object_id(self) :
		try:
			return super(rewrite_policy,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				rewrite_policy_obj=rewrite_policy()
				response = rewrite_policy_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of rewrite_policy resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return rewrite_policy_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the rewrite_policy resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = rewrite_policy_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of rewrite_policy resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = rewrite_policy_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(rewrite_policy_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(rewrite_policy_responses, response, "rewrite_policy_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.rewrite_policy_response_array
				i=0
				error = [rewrite_policy() for _ in range(len(response))]
				for obj in response :
			...
			response = result.rewrite_policy_response_array
			i=0
			rewrite_policy_objs = [rewrite_policy() for _ in range(len(response))]
			for obj in response :
				for props in obj._rewrite_policy:
					result = service.payload_formatter.string_to_bulk_resource(rewrite_policy_response,self.__class__.__name__,props)
					rewrite_policy_objs[i] = result.rewrite_policy
					i=i+1
			return rewrite_policy_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(rewrite_policy,self).validate()
		except Exception as e :
			raise e
...
'''
class rewrite_policy_response(base_response):
	def __init__(self,length=1) :
		self.rewrite_policy= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.rewrite_policy= [ rewrite_policy() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class rewrite_policy_responses(base_response):
	def __init__(self,length=1) :
		self.rewrite_policy_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.rewrite_policy_response_array = [ rewrite_policy() for _ in range(length)]
