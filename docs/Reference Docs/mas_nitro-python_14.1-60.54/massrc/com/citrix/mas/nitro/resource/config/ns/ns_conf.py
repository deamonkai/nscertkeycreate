...
class ns_conf(base_resource):
	...
	'''
	get Tells whether a diff exists between templates associated to NetScaler and the running configuration
	'''
	@property
	...
	'''
	Use this operation to download_conf.
	'''
	@classmethod
	def download_conf(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				res = resource.perform_operation(service,"download_conf")
				return res
			else : 
				ns_conf_obj= ns_conf()
				return cls.perform_operation_bulk_request(service,"download_conf",resource,ns_conf_obj)
		except Exception as e :
			raise e

	'''
	Use this operation to poll now.
	'''
	@classmethod
	def inventory(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				ns_conf_obj= ns_conf()
				return cls.perform_operation_bulk_request(service,"inventory",resource,ns_conf_obj)
		except Exception as e :
			raise e

	'''
	Get ns.conf information from NetScaler Instance.
	'''
	@classmethod
