...
class device(af_generic_api):
	...
	def get_object_id(self) :
		try:
			return super(device,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				device_obj=device()
				response = device_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of device resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return device_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the device resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = device_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of device resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = device_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(device_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(device_responses, response, "device_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.device_response_array
				i=0
				error = [device() for _ in range(len(response))]
				for obj in response :
			...
			response = result.device_response_array
			i=0
			device_objs = [device() for _ in range(len(response))]
			for obj in response :
				for props in obj._device:
					result = service.payload_formatter.string_to_bulk_resource(device_response,self.__class__.__name__,props)
					device_objs[i] = result.device
					i=i+1
			return device_objs
		except Exception as e :
			raise e
...
'''
class device_response(base_response):
	def __init__(self,length=1) :
		self.device= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.device= [ device() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class device_responses(base_response):
	def __init__(self,length=1) :
		self.device_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.device_response_array = [ device() for _ in range(length)]
