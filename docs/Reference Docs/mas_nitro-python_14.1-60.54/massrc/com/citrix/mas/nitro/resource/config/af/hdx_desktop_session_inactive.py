...
class hdx_desktop_session_inactive(af_generic_api):
	_city= ""
	_user_type= ""
	...
	def get_object_type(self) :
		try:
			return "hdx_desktop_session_inactive"
	...
	def get_object_id(self) :
		try:
			return super(hdx_desktop_session_inactive,self).get_object_id()
		except Exception as e :
			raise e
	...
	def file_component_value(self) :
		try :
			return "hdx_desktop_session_inactives"
		except Exception as e :
			raise e
	...
	'''
	L7 monitoring Supported or not
	'''
	@property
	...
	def l7_monitoring_supported(self,l7_monitoring_supported):
		try :
			if not isinstance(l7_monitoring_supported,float):
				raise TypeError("l7_monitoring_supported must be set to float value")
	...
	def count_usb_accepted(self,count_usb_accepted):
		try :
			if not isinstance(count_usb_accepted,float):
				raise TypeError("count_usb_accepted must be set to float value")
	...
	def ha_failover_count(self,ha_failover_count):
		try :
			if not isinstance(ha_failover_count,float):
				raise TypeError("ha_failover_count must be set to float value")
			self._ha_failover_count = ha_failover_count
		except Exception as e :
			raise e

	'''
	L7 Threshold Breach Count Value
	'''
	@property
	def l7_threshold_breach_count(self):
		try:
			return self._l7_threshold_breach_count
		except Exception as e :
			raise e
	'''
	L7 Threshold Breach Count Value
	'''
	@l7_threshold_breach_count.setter
	def l7_threshold_breach_count(self,l7_threshold_breach_count):
		try :
			if not isinstance(l7_threshold_breach_count,float):
				raise TypeError("l7_threshold_breach_count must be set to float value")
			self._l7_threshold_breach_count = l7_threshold_breach_count
		except Exception as e :
			raise e
	...
	def usb_status(self,usb_status):
		try :
			if not isinstance(usb_status,float):
				raise TypeError("usb_status must be set to float value")
			self._usb_status = usb_status
		except Exception as e :
			raise e

	'''
	No. of times USB Stopped
	'''
	@property
	...
	def count_usb_stopped(self,count_usb_stopped):
		try :
			if not isinstance(count_usb_stopped,float):
				raise TypeError("count_usb_stopped must be set to float value")
	...
	def count_usb_rejected(self,count_usb_rejected):
		try :
			if not isinstance(count_usb_rejected,float):
				raise TypeError("count_usb_rejected must be set to float value")
	...
	def session_terminate_time_local(self,session_terminate_time_local):
		try :
			if not isinstance(session_terminate_time_local,float):
				raise TypeError("session_terminate_time_local must be set to float value")
	...
	def terminated_reason(self,terminated_reason):
		try :
			if not isinstance(terminated_reason,str):
				raise TypeError("terminated_reason must be set to str value")
	...
	'''
	Af Report for HDX Desktop Session Inactive..
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				hdx_desktop_session_inactive_obj=hdx_desktop_session_inactive()
				response = hdx_desktop_session_inactive_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of hdx_desktop_session_inactive resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return hdx_desktop_session_inactive_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the hdx_desktop_session_inactive resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = hdx_desktop_session_inactive_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of hdx_desktop_session_inactive resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = hdx_desktop_session_inactive_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(hdx_desktop_session_inactive_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.hdx_desktop_session_inactive
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(hdx_desktop_session_inactive_responses, response, "hdx_desktop_session_inactive_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.hdx_desktop_session_inactive_response_array
				i=0
				error = [hdx_desktop_session_inactive() for _ in range(len(response))]
				for obj in response :
			...
			response = result.hdx_desktop_session_inactive_response_array
			i=0
			hdx_desktop_session_inactive_objs = [hdx_desktop_session_inactive() for _ in range(len(response))]
			for obj in response :
				for props in obj._hdx_desktop_session_inactive:
					result = service.payload_formatter.string_to_bulk_resource(hdx_desktop_session_inactive_response,self.__class__.__name__,props)
					hdx_desktop_session_inactive_objs[i] = result.hdx_desktop_session_inactive
					i=i+1
			return hdx_desktop_session_inactive_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(hdx_desktop_session_inactive,self).validate()
		except Exception as e :
			raise e
...
'''
class hdx_desktop_session_inactive_response(base_response):
	def __init__(self,length=1) :
		self.hdx_desktop_session_inactive= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.hdx_desktop_session_inactive= [ hdx_desktop_session_inactive() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class hdx_desktop_session_inactive_responses(base_response):
	def __init__(self,length=1) :
		self.hdx_desktop_session_inactive_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.hdx_desktop_session_inactive_response_array = [ hdx_desktop_session_inactive() for _ in range(length)]
