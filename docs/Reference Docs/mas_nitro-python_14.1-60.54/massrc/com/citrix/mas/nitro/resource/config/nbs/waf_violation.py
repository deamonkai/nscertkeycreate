...
'''
Configuration for WAF Violation resource
'''

class waf_violation(base_resource):
	_iprep_score= ""
	_city= ""
	_ip_address= ""
	_violation_location= ""
	...
	_counter_value= ""
	_violation_category= ""
	_iprep_category_desc= ""
	_severity_type= ""
	_violation_type_desc= ""
	_longitude= ""
	_http_method= ""
	_violation_value= ""
	_severity= ""
	...
	_appname= ""
	_violation_action= ""
	_violation_category_desc= ""
	_app_threat_index= ""
	_rpt_sample_time= ""
	_attack_category_desc= ""
	_block_flags= ""
	_transactionid= ""
	_attack_time= ""
	_region_code= ""
	...
	_iprep_category= ""
	_signature_category= ""
	_violation_location_desc= ""
	_violation_name= ""
	_violation_type= ""
	_total_attacks= ""
	_violation_action_desc= ""
	_country_code= ""
	_session_id= ""
	__count=""
	...
	def get_unprocessed_url(self) :
		try:
			return "/macro/v1/security/waf_violation"
		except Exception as e :
			raise e
	...
	'''
	get severity type description
	'''
	@property
	def severity_type_desc(self) :
		try:
			return self._severity_type_desc
		except Exception as e :
			raise e
	'''
	set severity type description
	'''
	@severity_type_desc.setter
	def severity_type_desc(self,severity_type_desc):
		try :
			if not isinstance(severity_type_desc,str):
				raise TypeError("severity_type_desc must be set to str value")
			self._severity_type_desc = severity_type_desc
		except Exception as e :
			raise e
	...
	'''
	@property
	def iprep_category_desc(self) :
		try:
			return self._iprep_category_desc
		except Exception as e :
			raise e
	'''
	set Description of category
	'''
	@iprep_category_desc.setter
	def iprep_category_desc(self,iprep_category_desc):
		try :
			if not isinstance(iprep_category_desc,str):
				raise TypeError("iprep_category_desc must be set to str value")
			self._iprep_category_desc = iprep_category_desc
		except Exception as e :
			raise e
	...
	def violation_type_desc(self,violation_type_desc):
		try :
			if not isinstance(violation_type_desc,str):
				raise TypeError("violation_type_desc must be set to str value")
	...
	def violation_category_desc(self) :
		try:
			...
			raise e
	'''
	set violation_category_desc.
	'''
	@violation_category_desc.setter
	def violation_category_desc(self,violation_category_desc):
		try :
			if not isinstance(violation_category_desc,str):
				raise TypeError("violation_category_desc must be set to str value")
	...
	'''
	get Attack Category Description
	'''
	@property
	def attack_category_desc(self) :
		try:
			return self._attack_category_desc
		except Exception as e :
			raise e
	'''
	set Attack Category Description
	'''
	@attack_category_desc.setter
	def attack_category_desc(self,attack_category_desc):
		try :
			if not isinstance(attack_category_desc,str):
				raise TypeError("attack_category_desc must be set to str value")
			self._attack_category_desc = attack_category_desc
		except Exception as e :
			raise e
	...
	'''
	get violation location description
	'''
	@property
	def violation_location_desc(self) :
		try:
			return self._violation_location_desc
		except Exception as e :
			raise e
	'''
	set violation location description
	'''
	@violation_location_desc.setter
	def violation_location_desc(self,violation_location_desc):
		try :
			if not isinstance(violation_location_desc,str):
				raise TypeError("violation_location_desc must be set to str value")
			self._violation_location_desc = violation_location_desc
		except Exception as e :
			raise e
	...
	'''
	get violation action description
	'''
	@property
	def violation_action_desc(self) :
		try:
			return self._violation_action_desc
		except Exception as e :
			raise e
	'''
	set violation action description
	'''
	@violation_action_desc.setter
	def violation_action_desc(self,violation_action_desc):
		try :
			if not isinstance(violation_action_desc,str):
				raise TypeError("violation_action_desc must be set to str value")
			self._violation_action_desc = violation_action_desc
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			response=""
			if not resource :
				waf_violation_obj=waf_violation()
				response = waf_violation_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of waf_violation resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return waf_violation_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the waf_violation resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = waf_violation_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of waf_violation resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = waf_violation_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(waf_violation_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(waf_violation_responses, response, "waf_violation_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.waf_violation_response_array
				i=0
				error = [waf_violation() for _ in range(len(response))]
				for obj in response :
			...
			response = result.waf_violation_response_array
			i=0
			waf_violation_objs = [waf_violation() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_waf_violation'):
					for props in obj._waf_violation:
						result = service.payload_formatter.string_to_bulk_resource(waf_violation_response,self.__class__.__name__,props)
						waf_violation_objs[i] = result.waf_violation
						i=i+1
			return waf_violation_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(waf_violation,self).validate()
		except Exception as e :
			raise e
...
'''
class waf_violation_response(base_response):
	def __init__(self,length=1) :
		self.waf_violation= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.waf_violation= [ waf_violation() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class waf_violation_responses(base_response):
	def __init__(self,length=1) :
		self.waf_violation_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.waf_violation_response_array = [ waf_violation() for _ in range(length)]
