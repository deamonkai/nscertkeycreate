...
class ns_config_replicate(base_resource):
	...
	'''
	get Source NetScaler IP Address
	'''
	@property
	def source_device(self) :
		try:
			...
			raise e
	'''
	set Source NetScaler IP Address
	'''
	...
	'''
	 Use this operation to replicate source ns device configuration to one or more target NetScaler instances.
	'''
	@classmethod
	def replicate(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				res = resource.perform_operation(service,"replicate")
				return res
			else : 
