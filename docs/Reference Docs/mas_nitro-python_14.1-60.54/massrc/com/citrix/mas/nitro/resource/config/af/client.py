...
class client(af_generic_api):
	...
	def get_object_id(self) :
		try:
			return super(client,self).get_object_id()
		except Exception as e :
			raise e
	...
	def network_latency_client_side_max(self,network_latency_client_side_max):
		try :
			if not isinstance(network_latency_client_side_max,float):
				raise TypeError("network_latency_client_side_max must be set to float value")
	...
	'''
	Af Report for Top Clients for this AF Collector.
	'''
	@classmethod
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				client_obj=client()
				response = client_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of client resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return client_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the client resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = client_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of client resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = client_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(client_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(client_responses, response, "client_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.client_response_array
				i=0
				error = [client() for _ in range(len(response))]
				for obj in response :
			...
			response = result.client_response_array
			i=0
			client_objs = [client() for _ in range(len(response))]
			for obj in response :
				for props in obj._client:
					result = service.payload_formatter.string_to_bulk_resource(client_response,self.__class__.__name__,props)
					client_objs[i] = result.client
					i=i+1
			return client_objs
		except Exception as e :
			raise e
...
'''
class client_response(base_response):
	def __init__(self,length=1) :
		self.client= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.client= [ client() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class client_responses(base_response):
	def __init__(self,length=1) :
		self.client_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.client_response_array = [ client() for _ in range(length)]
