...
class hdx_desktop(base_resource):
	_client_srtt= ""
	_server_latency= ""
	...
	'''
	@property
	def active_session_duration(self):
		try:
			return self._active_session_duration
		except Exception as e :
			raise e
	'''
	active session duration in seconds.
	'''
	@active_session_duration.setter
	def active_session_duration(self,active_session_duration):
		try :
			if not isinstance(active_session_duration,float):
				raise TypeError("active_session_duration must be set to float value")
			self._active_session_duration = active_session_duration
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				hdx_desktop_obj=hdx_desktop()
				response = hdx_desktop_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of hdx_desktop resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return hdx_desktop_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the hdx_desktop resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = hdx_desktop_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of hdx_desktop resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = hdx_desktop_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(hdx_desktop_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(hdx_desktop_responses, response, "hdx_desktop_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.hdx_desktop_response_array
				i=0
				error = [hdx_desktop() for _ in range(len(response))]
				for obj in response :
			...
			response = result.hdx_desktop_response_array
			i=0
			hdx_desktop_objs = [hdx_desktop() for _ in range(len(response))]
			for obj in response :
				for props in obj._hdx_desktop:
					result = service.payload_formatter.string_to_bulk_resource(hdx_desktop_response,self.__class__.__name__,props)
					hdx_desktop_objs[i] = result.hdx_desktop
					i=i+1
			return hdx_desktop_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(hdx_desktop,self).validate()
		except Exception as e :
			raise e
...
'''
class hdx_desktop_response(base_response):
	def __init__(self,length=1) :
		self.hdx_desktop= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.hdx_desktop= [ hdx_desktop() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class hdx_desktop_responses(base_response):
	def __init__(self,length=1) :
		self.hdx_desktop_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.hdx_desktop_response_array = [ hdx_desktop() for _ in range(length)]
