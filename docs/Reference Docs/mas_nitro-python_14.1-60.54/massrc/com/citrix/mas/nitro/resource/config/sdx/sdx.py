...
from massrc.com.citrix.mas.nitro.resource.config.sdx.vpx_info import vpx_info
...
class sdx(host_device):
	...
	'''
	@property
	def lom_ip_address(self) :
		try:
			return self._lom_ip_address
		except Exception as e :
			raise e
	'''
	set LOM IP Address
	'''
	@lom_ip_address.setter
	def lom_ip_address(self,lom_ip_address):
		try :
			if not isinstance(lom_ip_address,str):
				raise TypeError("lom_ip_address must be set to str value")
			self._lom_ip_address = lom_ip_address
		except Exception as e :
			raise e
	...
	'''
	get Get BW of VPX on SDX info
	'''
	@property
	def vpx_information(self) :
		try:
			return self._vpx_information
		except Exception as e :
			raise e
	'''
	set Get BW of VPX on SDX info
	'''
	@vpx_information.setter
	def vpx_information(self,vpx_information) :
		try :
			if not isinstance(vpx_information,list):
				raise TypeError("vpx_information must be set to array of vpx_info value")
			for item in vpx_information :
				if not isinstance(item,vpx_info):
					raise TypeError("item must be set to vpx_info value")
			self._vpx_information = vpx_information
		except Exception as e :
			raise e
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				sdx_obj= sdx()
				return cls.perform_operation_bulk_request(service,resource,sdx_obj)
		except Exception as e :
			raise e
