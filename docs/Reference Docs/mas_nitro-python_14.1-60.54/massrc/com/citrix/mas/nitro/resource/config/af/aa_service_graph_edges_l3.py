...
'''
Configuration for Service Graph Edges Report table for Level 3 resource
'''

class aa_service_graph_edges_l3(base_resource):
	_error_count= ""
	_source_namespace= ""
	_rec_type= ""
	_srvr_cert_hash= ""
	...
	def get_object_type(self) :
		try:
			return "aa_service_graph_edges_l3"
	...
	def file_component_value(self) :
		try :
			return "aa_service_graph_edges_l3s"
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				aa_service_graph_edges_l3_obj=aa_service_graph_edges_l3()
				response = aa_service_graph_edges_l3_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of aa_service_graph_edges_l3 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return aa_service_graph_edges_l3_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the aa_service_graph_edges_l3 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = aa_service_graph_edges_l3_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of aa_service_graph_edges_l3 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = aa_service_graph_edges_l3_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(aa_service_graph_edges_l3_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.aa_service_graph_edges_l3
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(aa_service_graph_edges_l3_responses, response, "aa_service_graph_edges_l3_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.aa_service_graph_edges_l3_response_array
				i=0
				error = [aa_service_graph_edges_l3() for _ in range(len(response))]
				for obj in response :
			...
			response = result.aa_service_graph_edges_l3_response_array
			i=0
			aa_service_graph_edges_l3_objs = [aa_service_graph_edges_l3() for _ in range(len(response))]
			for obj in response :
				for props in obj._aa_service_graph_edges_l3:
					result = service.payload_formatter.string_to_bulk_resource(aa_service_graph_edges_l3_response,self.__class__.__name__,props)
					aa_service_graph_edges_l3_objs[i] = result.aa_service_graph_edges_l3
					i=i+1
			return aa_service_graph_edges_l3_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(aa_service_graph_edges_l3,self).validate()
		except Exception as e :
			raise e
...
'''
class aa_service_graph_edges_l3_response(base_response):
	def __init__(self,length=1) :
		self.aa_service_graph_edges_l3= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.aa_service_graph_edges_l3= [ aa_service_graph_edges_l3() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class aa_service_graph_edges_l3_responses(base_response):
	def __init__(self,length=1) :
		self.aa_service_graph_edges_l3_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.aa_service_graph_edges_l3_response_array = [ aa_service_graph_edges_l3() for _ in range(length)]
