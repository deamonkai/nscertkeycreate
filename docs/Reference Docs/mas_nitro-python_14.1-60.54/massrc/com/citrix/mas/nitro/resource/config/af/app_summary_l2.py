...
'''
Configuration for APP Report table for Level 2 resource
'''

class app_summary_l2(base_resource):
	_hostname= ""
	_vservertype= ""
	_total_bytes= ""
	_total_requests= ""
	...
	def health_score(self,health_score):
		try :
			if not isinstance(health_score,int):
				raise TypeError("health_score must be set to int value")
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				app_summary_l2_obj=app_summary_l2()
				response = app_summary_l2_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of app_summary_l2 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return app_summary_l2_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the app_summary_l2 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = app_summary_l2_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of app_summary_l2 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = app_summary_l2_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(app_summary_l2_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(app_summary_l2_responses, response, "app_summary_l2_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.app_summary_l2_response_array
				i=0
				error = [app_summary_l2() for _ in range(len(response))]
				for obj in response :
			...
			response = result.app_summary_l2_response_array
			i=0
			app_summary_l2_objs = [app_summary_l2() for _ in range(len(response))]
			for obj in response :
				for props in obj._app_summary_l2:
					result = service.payload_formatter.string_to_bulk_resource(app_summary_l2_response,self.__class__.__name__,props)
					app_summary_l2_objs[i] = result.app_summary_l2
					i=i+1
			return app_summary_l2_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(app_summary_l2,self).validate()
		except Exception as e :
			raise e
...
'''
class app_summary_l2_response(base_response):
	def __init__(self,length=1) :
		self.app_summary_l2= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.app_summary_l2= [ app_summary_l2() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class app_summary_l2_responses(base_response):
	def __init__(self,length=1) :
		self.app_summary_l2_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.app_summary_l2_response_array = [ app_summary_l2() for _ in range(length)]
