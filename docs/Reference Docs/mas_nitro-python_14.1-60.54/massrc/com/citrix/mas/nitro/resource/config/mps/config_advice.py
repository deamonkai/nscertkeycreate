...
class config_advice(base_resource):
	...
	'''
	Use this operation to apply selected commands on the device.
	'''
	@classmethod
	def custom(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				res = resource.perform_operation(service,"custom")
				return res
			else : 
	...
	def download(cls,service=None,resource=None):
		try:
			...
			if resource is None :
				raise Exception("Resource Object Not Found")
			if type(resource) is not list :
				res = resource.perform_operation(service,"download")
				return res
			else : 
				config_advice_obj= config_advice()
