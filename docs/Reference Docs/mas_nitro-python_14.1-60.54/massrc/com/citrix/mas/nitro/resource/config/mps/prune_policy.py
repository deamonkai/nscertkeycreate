...
class prune_policy(base_resource):
	...
	'''
	get True, if user wants to opt for auto DB data purging once disk usage reaches threshold value
	'''
	@property
	def do_auto_purge(self) :
		try:
			return self._do_auto_purge
		except Exception as e :
			raise e
	'''
	set True, if user wants to opt for auto DB data purging once disk usage reaches threshold value
	'''
	@do_auto_purge.setter
	def do_auto_purge(self,do_auto_purge):
		try :
			if not isinstance(do_auto_purge,bool):
				raise TypeError("do_auto_purge must be set to bool value")
			self._do_auto_purge = do_auto_purge
		except Exception as e :
			raise e
	...
	'''
	get Disk Pruning Threshold
	'''
	@property
	def disk_prune_threshold(self) :
		try:
			return self._disk_prune_threshold
		except Exception as e :
			raise e
	'''
	set Disk Pruning Threshold
	'''
	@disk_prune_threshold.setter
	def disk_prune_threshold(self,disk_prune_threshold):
		try :
			if not isinstance(disk_prune_threshold,int):
				raise TypeError("disk_prune_threshold must be set to int value")
			self._disk_prune_threshold = disk_prune_threshold
		except Exception as e :
			raise e
