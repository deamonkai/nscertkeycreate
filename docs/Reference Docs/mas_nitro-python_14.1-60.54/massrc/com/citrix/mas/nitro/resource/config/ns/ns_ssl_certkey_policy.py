...
class ns_ssl_certkey_policy(base_resource):
	...
	'''
	This flag is to trigger migration of NetScaler certificates to cert_store.
	'''
	@property
	def cert_store_migration(self):
		try:
			return self._cert_store_migration
		except Exception as e :
			raise e
	'''
	This flag is to trigger migration of NetScaler certificates to cert_store.
	'''
	@cert_store_migration.setter
	def cert_store_migration(self,cert_store_migration):
		try :
			if not isinstance(cert_store_migration,bool):
				raise TypeError("cert_store_migration must be set to bool value")
			self._cert_store_migration = cert_store_migration
		except Exception as e :
			raise e

	'''
	Activity ID that is to track the polling status using resource poll_activity_status.
	'''
	@property
	def act_id(self):
		try:
