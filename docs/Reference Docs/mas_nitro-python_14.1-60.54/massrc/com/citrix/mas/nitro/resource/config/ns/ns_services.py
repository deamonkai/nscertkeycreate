...
'''
Configuration for NetScaler Service resource
'''

class ns_services(base_resource):
	_id= ""
	_svc_type= ""
	_svr_name= ""
	_svc_state= ""
	...
	'''
	get Server Name associated with service
	'''
	@property
	def svr_name(self) :
		try:
	...
	def publicip(self,publicip):
		try :
			if not isinstance(publicip,str):
				raise TypeError("publicip must be set to str value")
	...
	'''
	Use this operation to poll the service.
	'''
	@classmethod
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ns_services_obj=ns_services()
				response = ns_services_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ns_services resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ns_services_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ns_services resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ns_services_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ns_services resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ns_services_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['___count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_services_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_services_responses, response, "ns_services_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ns_services_response_array
				i=0
				error = [ns_services() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ns_services_response_array
			i=0
			ns_services_objs = [ns_services() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_ns_services'):
					for props in obj._ns_services:
						result = service.payload_formatter.string_to_bulk_resource(ns_services_response,self.__class__.__name__,props)
						ns_services_objs[i] = result.ns_services
						i=i+1
			return ns_services_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ns_services,self).validate()
		except Exception as e :
			raise e
...
'''
class ns_services_response(base_response):
	def __init__(self,length=1) :
		self.ns_services= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ns_services= [ ns_services() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ns_services_responses(base_response):
	def __init__(self,length=1) :
		self.ns_services_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ns_services_response_array = [ ns_services() for _ in range(length)]
