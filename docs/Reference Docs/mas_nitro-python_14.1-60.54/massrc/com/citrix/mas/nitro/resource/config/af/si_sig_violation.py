...
class si_sig_violation(af_generic_api):
	_block_flags= ""
	...
	def get_object_id(self) :
		try:
			return super(si_sig_violation,self).get_object_id()
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				si_sig_violation_obj=si_sig_violation()
				response = si_sig_violation_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of si_sig_violation resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return si_sig_violation_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the si_sig_violation resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = si_sig_violation_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of si_sig_violation resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = si_sig_violation_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(si_sig_violation_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.si_sig_violation
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(si_sig_violation_responses, response, "si_sig_violation_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.si_sig_violation_response_array
				i=0
				error = [si_sig_violation() for _ in range(len(response))]
				for obj in response :
			...
			response = result.si_sig_violation_response_array
			i=0
			si_sig_violation_objs = [si_sig_violation() for _ in range(len(response))]
			for obj in response :
				for props in obj._si_sig_violation:
					result = service.payload_formatter.string_to_bulk_resource(si_sig_violation_response,self.__class__.__name__,props)
					si_sig_violation_objs[i] = result.si_sig_violation
					i=i+1
			return si_sig_violation_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(si_sig_violation,self).validate()
		except Exception as e :
			raise e
...
'''
class si_sig_violation_response(base_response):
	def __init__(self,length=1) :
		self.si_sig_violation= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.si_sig_violation= [ si_sig_violation() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class si_sig_violation_responses(base_response):
	def __init__(self,length=1) :
		self.si_sig_violation_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.si_sig_violation_response_array = [ si_sig_violation() for _ in range(length)]
