...
'''
Configuration for AF HDX Active Session details Report table for Level 4 resource
'''

class af_hdx_active_session_details_l4(base_resource):
	_session_setup_time= ""
	_sr_reconnect_count= ""
	_exporter_id= ""
	_latitude= ""
	...
	def get_object_type(self) :
		try:
			return "af_hdx_active_session_details_l4"
	...
	def file_component_value(self) :
		try :
			return "af_hdx_active_session_details_l4s"
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				af_hdx_active_session_details_l4_obj=af_hdx_active_session_details_l4()
				response = af_hdx_active_session_details_l4_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of af_hdx_active_session_details_l4 resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return af_hdx_active_session_details_l4_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the af_hdx_active_session_details_l4 resources configured on NetScaler SDX.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = af_hdx_active_session_details_l4_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of af_hdx_active_session_details_l4 resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = af_hdx_active_session_details_l4_obj.getfiltered(service, option_)
			if response :
				return response[0].__dict__['__count']
			return 0;
		except Exception as e :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_hdx_active_session_details_l4_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.af_hdx_active_session_details_l4
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(af_hdx_active_session_details_l4_responses, response, "af_hdx_active_session_details_l4_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.af_hdx_active_session_details_l4_response_array
				i=0
				error = [af_hdx_active_session_details_l4() for _ in range(len(response))]
				for obj in response :
			...
			response = result.af_hdx_active_session_details_l4_response_array
			i=0
			af_hdx_active_session_details_l4_objs = [af_hdx_active_session_details_l4() for _ in range(len(response))]
			for obj in response :
				for props in obj._af_hdx_active_session_details_l4:
					result = service.payload_formatter.string_to_bulk_resource(af_hdx_active_session_details_l4_response,self.__class__.__name__,props)
					af_hdx_active_session_details_l4_objs[i] = result.af_hdx_active_session_details_l4
					i=i+1
			return af_hdx_active_session_details_l4_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(af_hdx_active_session_details_l4,self).validate()
		except Exception as e :
			raise e
...
'''
class af_hdx_active_session_details_l4_response(base_response):
	def __init__(self,length=1) :
		self.af_hdx_active_session_details_l4= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.af_hdx_active_session_details_l4= [ af_hdx_active_session_details_l4() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class af_hdx_active_session_details_l4_responses(base_response):
	def __init__(self,length=1) :
		self.af_hdx_active_session_details_l4_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.af_hdx_active_session_details_l4_response_array = [ af_hdx_active_session_details_l4() for _ in range(length)]
